...
* [Run phases](#run-phases)
  * [Test binary selection](#test-binary-selection)
...
* [Killing mutants](#killing-mutants)
  * [Fixing timeouts, out-of-memory and unviable mutants](#fixing-timeouts-out-of-memory-and-unviable-mutants)
    * [Taking them back out](#taking-them-back-out)
  * [Hangs](#hangs)
...
* [Migrating from cargo-mutants](#migrating-from-cargo-mutants)
  * [Comparing the numbers](#comparing-the-numbers)
...
Four references live beside this file:
...
[docs/gamma.toml](docs/gamma.toml) is a fully documented configuration file to copy as a
starting point.
...
* **Surviving mutant (⚠️):** All tests passed. Your test suite didn't notice the change, pointing out a
...
`cargo-gamma` brings fast, comprehensive mutation testing to Rust projects of all sizes. But there are other
...
* **Speed.** Traditional mutation testers rebuild your crate for every single mutant, which can take hours or
  days. `cargo-gamma` compiles every mutant into a single instrumented test binary and activates them
...
actually change your original source code as all the work `cargo-gamma` does happens on copied state in your
target directory). Once your codebase is rebuilt, your test suite is executed repeatedly, each run with a
...
The default selection enables the main catalog; additional low-yield mutators live in `@pedantic`.
A [mutator preset](docs/MUTATORS.md#mutator-presets) groups the catalog by what a mutant disturbs, so a narrower
audit is one word rather than a list you have to maintain. For example, `@control` is everything that
changes which code runs rather than what it computes, and `@numeric` is literal replacement and expression
perturbation:
...
```mermaid
flowchart LR
    D[1. Discovery<br/><i>Find mutation sites</i>] --> I[2. Instrumentation<br/><i>Inject mutations</i>]
...
|---|---|---|
| **Discovery** | Parse source tree and identify mutation sites.<br>Suppressions and slices apply here. | Seconds |
| **Instrumentation** | Copy workspace to temporary location and introduce<br>all mutations. | Seconds |
| **Building** | Compile instrumented tree.<br>Unviable mutants are eliminated. | Seconds or Minutes - full debug build |
...
The set of test binaries allowed to decide a verdict is capped at the ones `cargo test` would run
where you invoked `cargo-gamma`. Running inside a crate judges its mutants by that crate's own
suite, exactly as every other cargo command scopes itself to the crate you are standing in — so the
cost of a run is a multiple of the cost of a bare `cargo test`, and the score is a property of the
crate rather than of whatever else in the workspace happens to link it. `--test-package` and
`--test-workspace` widen the oracle when you want more; see [Scoping a run](#scoping-a-run).

Within that cap, a test binary built from a package that does not depend, directly or transitively,
on the package a mutant lives in can never kill that mutant, so `cargo-gamma` does not build or run
it — which considerably reduces overall test time.

A mutant that no admitted test binary can reach is reported as `uncovered` rather than as a
survivor: nothing tested it, which is a different finding from a test having tried and missed.
...
The oracle is capped at the tests `cargo test` would run where you invoked it. Run `cargo gamma`
inside `ledger-core` and its own suite decides every verdict; the crates that depend on it are not
compiled, not run, and not consulted. This is what keeps a run's cost a multiple of a bare
`cargo test` rather than a function of the reverse-dependency graph.
...
To widen the oracle, ask for it. `--test-package` adds the packages whose tests should judge, which
is what a workspace that keeps its integration tests in a package of their own wants; if a run finds
no test at all reaching the mutated code, it names the packages that could and suggests the flag.
...
the testing phase when use in to handle pull requests in a CI environment:
...
A `trybuild` or `compiletest_rs` target is the expensive special case, and a run says so before it
builds anything:
...
cargo gamma run --killed                      # list what the suite killed, not just what survived
...
cargo gamma run --incremental full          # default: reuse unviability and settled test verdicts
cargo gamma run --incremental build         # reuse only compiler unviability
...
As soon as a measured run acquires that directory, it truncates its `testing_progress.log` (normally
`target/gamma/testing_progress.log`). Every mutant verdict is appended and flushed there as it
arrives, using the console's outcome-line format but without color or redraw escapes. Unlike the
console, the journal includes ordinary killed mutants as well as survivors, timeouts, memory kills,
...
The default report destinations follow it: they are written under the same gamma directory, so a
read-only checkout still gets its reports.

Every run writes what it learned to `target/gamma/last-run.json`: the verdict it reached for each
mutant, the test that did the killing, and what each cost. A repeated run reads it back (enabled by
default with `--incremental full`), so it does not spend rebuild rounds rediscovering what will not
compile and goes straight to the test that caught each mutant instead of paying for everything ahead
of it. Reuse is deliberately conservative: it requires the same compiler, Cargo configuration, and
verdict policy plus an unchanged pre-run snapshot of every workspace Cargo input; timeouts are
always rerun. See
[What a run remembers](#what-a-run-remembers) for the details; `--incremental no` performs a cold
run. `cargo gamma hints` promotes the parts that cannot move a score into `.cargo/gamma-hints.json`,
...
Every mutant ends in one of ten verdicts. Three count as detected, two as undetected, and five are
excluded from scoring entirely:

```mermaid
flowchart TD
    Start([Mutant Candidate]) --> Filter{Active in run?}
    Filter -- Suppressed / out-of-shard --> Ignored[Ignored / Not Built<br/><i>(excluded)</i>]
    Filter -- Yes --> Compiles{Compiles?}
    Compiles -- No --> Unviable[Unviable<br/><i>(excluded)</i>]
    Compiles -- Yes --> Census{Reached by tests?}
    Census -- No --> Uncovered[Uncovered ⚠️<br/><i>(undetected)</i>]
    Census -- Yes --> TestRun[Execute test suite]
    TestRun --> Outcome{Test outcome}
    Outcome -- Test fails --> Killed[Killed ✅<br/><i>(detected)</i>]
    Outcome -- Hangs / stalls --> Timeout[Timed Out ⏱️<br/><i>(detected)</i>]
    Outcome -- Exceeds memory --> OutOfMem[Out of Memory 💥<br/><i>(detected)</i>]
    Outcome -- All tests pass --> Survived[Survived ⚠️<br/><i>(undetected)</i>]
```

| Verdict | In the report | Meaning | In the score |
|---|---|---|---|
| Killed | `killed` | A test failed while this mutant was active | detected |
| Timed out | `timeout` | The budget or the stall detector cut it off | detected |
| Out of memory | `outofmem` | It passed the memory ceiling derived from the baseline | detected |
...
```text
             killed + timed out + out of memory
score =  ──────────────────────────────────────────────  × 100
...
For example, the first summary above has 51 detected mutants and 32 undetected mutants:

```text
51 / (51 + 32) × 100 = 61.445...%, displayed as 61.4%
```

A timeout or out-of-memory verdict increases both the numerator and denominator. A survivor or
uncovered verdict increases only the denominator. An excluded verdict changes neither. If there
are no scored mutants at all, the displayed ratio is 100%, but `--min-score` treats the run as
ungraded rather than allowing an empty population to pass a quality gate.
...
This is also the [mutation-testing-elements](https://github.com/stryker-mutator/mutation-testing-elements)
schema's own definition, so this number and the one the standard report UI computes agree exactly.
...
They are the reason a mutation score of 100% is usually neither achievable nor a sensible target.
Deciding whether a given mutant is equivalent is undecidable in general, so no tool can filter them
out for you; a survivor is a question addressed to you, not a defect report.

Three things keep the cost down in practice:

* Mutants that fail to compile are withdrawn automatically as **unviable**, which removes a large
  class of useless survivors before they ever reach the score.
...
answer. `unviable` stays opt-in: a mutant that will not compile is cheap to re-establish, and
suppressing it hides a mutator that may simply need narrowing.
...
```
     TIMEOUT src/parse.rs:88:11: replace remaining > 0 with (remaining) >= (0) [relational.gt_to_ge]:
             stalled, last test named was `tests::round_trip`
       Hangs a mutant is cut off after 5.0s of silence; the baseline'"'"'s longest was 0.4s
```

The figure at the end is what was spent, not what was allowed: a mutant is stopped shortly after its
budget runs out, and a suspected timeout is re-run with more room before it is believed. It is there
to tell a genuine hang from a budget calibrated too tightly, which otherwise print the same line and
...
actually spinning is usually one it has not got round to naming. Re-running with
`-- --test-threads=1` makes the name exact, at the cost of the parallelism. A timeout counts as
detected — a hang is a behavior change the suite noticed, just expensively — so hung mutants are
listed explicitly rather than disappearing into the killed total.
`cargo gamma suppress` writes suppressions for them, and
`--no-stall-detection` restores waiting out the full budget.
...
Summary 1 mutant (0 killed, 0 survived, 0 timed out, 1 out of memory, 0 uncovered => 100.0%)
```
...
being folded into `killed` because the suite's assertions did not fail — the kernel stopped the
...
starts a server or a nested build.

On Linux, a cgroup is delegated to a process only when something arranged it, which a systemd user
session does and most other things do not — so a shell inside a container, a CI runner, `docker
exec`, or a terminal spawned by an editor typically starts out unable to bound anything. When that
is the case, `cargo-gamma` asks the systemd user manager for a scope of its own and re-runs itself
inside it, saying so as it does:

```
Note re-running inside a systemd scope, because this host does not otherwise delegate a cgroup for memory control.
     Pass `--no-relaunch` to run without it
```

The relaunched process is the one that does the work, and its exit code is the one you get. Nothing
else about the run changes. Pass `--no-relaunch` to stay in the original process — worth doing where
a new process is itself the problem, such as a supervisor tracking this pid — and the run then
follows the rules below for a host that cannot enforce.

Arranging the scope yourself has the same effect, and is what to do when the relaunch is unwanted
but the ceiling is not:
...
| `// gamma::skip` | the same, without an attribute | beside code | site cannot carry an attribute or in clean source |
...
Both spellings above are accepted and mean the same thing: the `#[` and `]` are optional in a
comment directive, so `// gamma::skip(arith)` and `// #[gamma::skip(arith)]` resolve identically.

A misspelled `gamma::` directive is a usage error rather than a silent no-op — in either spelling —
because a directive that reads as if it works and does nothing is the one failure mode that hides
real survivors. A comment only has to parse as a directive when it *starts* with `gamma::` or
`mutants::`; prose that merely mentions one is left alone.
...
An `expect_survived` mutant that gets killed is reported, and so is an `expect_killed` mutant that
...
```bash
cp docs/gamma.toml .cargo/gamma.toml
```
...
`.cargo/mutants.toml` is never read; see [Migrating from cargo-mutants](#migrating-from-cargo-mutants) for
how to translate one.

## Migrating from cargo-mutants

`#[mutants::skip]` is not honored — it is another tool's attribute, and reading it would mean this
tool's exclusions were partly written in a vocabulary it does not own. It is ignored rather than
rejected, so a codebase still carrying one builds and runs exactly as before; the sites it named
simply come back as mutants again. Rewrite each one as `#[gamma::skip]`, which takes the same bare
form and adds selectors, a `reason` and a `tag`.

`.cargo/mutants.toml` is deliberately
**not** read — it is a different schema for a different tool, and honoring it silently would let
...
Nothing is dropped, but the two ways a key can fail to translate read differently. A key this tool
recognizes and has deliberately settled — because gamma already does the thing unconditionally, or
because the setting moved to the command line — becomes a comment explaining that, so you know it
needs no action. A key with no equivalent at all becomes a `TODO` comment carrying the original
text. Every translated line names the key it came from.
...
larger population almost always means a lower score for the same suite.

Narrowing the selection to the families cargo-mutants also mutates brings the two populations
closer:

```bash
cargo gamma run --mutators \
    fn_value,relational,logical,arith,bitwise,shift,assign,unary,match_arm,match_guard,struct_field
```

That is an approximation rather than a match, and the residual differences are worth knowing before
reading anything into the comparison:

* **The replacement tables differ inside those families.** cargo-mutants deliberately never turns
  `==` into `<` or `<=`, and never turns `&=` into `^=`, because both are prone to uninformative
  mutants. This catalog swaps orderings among themselves and has no `==`-to-ordering mutator at
  all, and `match_guard.negate` has no counterpart there.
* **`unsafe` functions.** cargo-mutants excludes them from mutation entirely. This tool does not,
  so it mutates a region the other never reaches.
* **Doctests.** They are never built or run here, so a mutant whose only coverage is a doc-comment
  example is reported `uncovered` or `survived` where cargo-mutants would catch it. This one moves
  the score rather than merely the population.

For functions returning `impl Iterator`, both tools mutate the return value by different means:
cargo-mutants replaces the whole body, so its `core::iter::empty()` is the only type in play,
while a guard here shares an `if` with the original. Each arm is wrapped in a variant of
`gamma_rt::Either`, which is one type whatever the two sides are, and which keeps `Send`, `Sync`
and `Clone` because they are derived from the two sides rather than erased. A `Box<dyn Iterator>`
would have unified the arms just as well and broken every signature that promised one of those.

Once the two agree, drop the narrowing and let the rest of the catalog tell you what the smaller
...
```bash
cargo gamma run                                        # both reports land in target/gamma
cargo gamma run --html-report r.html --json-report r.json    # or somewhere you choose
```

Every run writes both reports without being asked. They land next to the run's other state, under
`target/gamma/report.html` and `target/gamma/report.json` — or under the [`--scratch-dir`](#controlling-a-run)
when one is set, since that is the directory the run is guaranteed to be able to write to. The
flags do not switch the reports on; they only move them somewhere you would rather have them.

The reason is that the flags were a trap. A run that takes an hour and answers a question about
your test suite is exactly the run you do not want to repeat because you forgot to ask for the file
that answers it. Writing them costs a fraction of a second against that hour, and `target` is
already ignored by git, so nothing is cluttered by the default.

The [SARIF log](#continuous-integration) is the exception and stays opt-in: it is addressed to a
particular code-scanning service rather than to you, and uploading it is a deliberate act.
...
`target/gamma/last-run.json` only ever costs time, and `--incremental no` ignores it for one run.
...
move a score. That is also why it needs no invalidation, unlike a carried verdict — a stale entry
costs one filtered test process and is then dropped.
...
record on purpose: a feature change discards every verdict, and discarding the hints along with them
...
The active Cargo profile is part of a run's build and verdict context. Changing it invalidates
build and verdict reuse, and the changed code generation can change which mutants tests catch even
...
The profile is documentation rather than generated setup: adding a Cargo profile changes the
project's own build policy, so `migrate` and cargo-gamma do not edit `Cargo.toml` automatically.
...
cargo gamma hints                     # write .cargo/gamma-hints.json
```

Later runs read `.cargo/gamma-hints.json` automatically without any command-line flags. Only two
...
Verdicts are refused entry, and that is the whole line the file walks. A kill adopted quietly out of
version control would settle part of your score from somebody else's run, whereas incremental mode
relies on local workspace source and test file hashing to guarantee sound verdicts.

The file is a promotion rather than a copy, so the tool owns its format version and provenance,
drops entries for mutants that no longer exist, orders them by source file and then by content ID so
the diff is reviewable, writes it atomically and reads back what it wrote. Regenerate it whenever it
is convenient — a nightly job, or by hand after a big change. Deleting it, or letting it go stale,
costs time and never signal; a corrupt or foreign file is treated exactly like a missing one.

The run says what the ordering hints actually bought, and does not guess at what they saved:

```text
hints     412 mutants front-loaded over 1 probe round, 398 confirmed unviable by the compiler
```

Front-loaded against confirmed is the number that tells you whether the file is still earning its
place. Rounds *saved* is not reported, because that would be the length of a build that never
happened.
...
The cost is one process per test, once, before the sweep starts. Under a suite of a few thousand
tests that is seconds; what it buys back is paid on every mutant.
...
reported as surviving. Where the measurement is visibly incomplete the run does not guess: a binary
that could not be listed, or whose census was cut short, is thrown away whole and its mutants run
its whole suite as before.

### Incremental runs

During development, picking up where a previous run left off avoids re-testing already-settled
mutants:

```bash
cargo gamma run --incremental full          # default: reuses unviability and settled test verdicts
cargo gamma run --incremental build         # reuses compiler unviability only
cargo gamma run --incremental no            # cold run: build and test everything from scratch
```

In `full` mode (the default), earlier kills, unviable mutants, and ignored mutants are skipped only
when the compiler, Cargo configuration, verdict policy, and complete pre-run snapshot of workspace
Cargo inputs match. Timeouts, survivors, and flakes are always retried. A killer must still have the
same exact identity in the same unchanged declaring file, so a modified, deleted, renamed, or
ambiguous killer is rechecked rather than carried blindly.
...
flowchart TD
    subgraph Run1 [Run N: Execution]
        ...
        B --> C[(target/gamma/last-run.json<br/><i>Local run cache</i>)]
    end

    subgraph Hints [Version Control]
        C -->|cargo gamma hints| D[(.cargo/gamma-hints.json<br/><i>Checked-in hints</i>)]
    end

    subgraph Run2 [Run N+1 / Fresh CI: Accelerated Sweep]
        D --> E[Run known killer test first]
        D --> F[Front-load unviable mutants]
        C -->|--incremental full| G[Skip already settled mutants]
    end
```

| | Lives in | Lasts | Believed |
|---|---|---|---|
| The run record | `target/gamma/last-run.json` | Until scratch tree is deleted | Unviability in `build` mode; unviability & settled verdicts in `full` mode (validated by complete workspace snapshots) |
| The hints file | `.cargo/gamma-hints.json` | Forever, and through review | Never — every hint is checked |
| A skip directive | Your source | Forever, and through review | Always — you wrote it |

The record is the tool's own memory and it is written on every run. It holds kills, unviable
mutants, and deliberate skips, with the killing test's declaring file and a cryptographic snapshot
of every workspace Cargo input. The snapshot covers source and test modules, manifests, lockfiles,
build scripts, and configuration while excluding generated target and scratch trees. It is keyed by
the mutant's content id, which hashes the file, the item path, the mutator and the replacement.

By default a run adopts only the part of it that cannot move a score: the mutants that would not
compile. Those are excluded from the score either way, so carrying them can only save time.
...
not — is a convenience that must be safe to delete: `rm -rf target/gamma` and
`rm .cargo/gamma-hints.json` cost you time and nothing else.
...
cargo gamma run --advice advice.md    # run, then write down where the time went, here
```
...
The advice document is written on every run, to `target/gamma/advice.md`; `--advice` only chooses
a different destination. It turns a finished run into a Markdown document: a list of findings, each a measured
...
`target/gamma/diagnostics.json`: one machine-readable record of what the run did, small enough to
attach to an issue.

```bash
cargo gamma run --diag-bundle bundle.json   # write it somewhere else
...
| cold CI runs starting from scratch | check in `.cargo/gamma-hints.json` with `cargo gamma hints` | nothing |
...
```bash
cargo gamma run --sarif-report mutants.sarif  # then upload with github/codeql-action/upload-sarif
```
      ...
      - name: Run mutation testing on changed files
        run: |
          git diff origin/main | cargo gamma run \
            --in-diff - \
            --min-score 80 \
            --sarif-report target/mutants.sarif

      - name: Upload SARIF report
        uses: github/codeql-action/upload-sarif@v3
        if: always()
        with:
          sarif_file: target/mutants.sarif
