...
mod summary {
    ...
    /// timeout 4, out-of-memory 2 in the numerator, survived 3 and uncovered 9 in the denominator
    /// only — so that moving *any* single outcome across the line changes the asserted number: the
    /// numerator would read differently, or the denominator would shrink, and 52.0% would not
    /// survive it. The non-scoring outcomes carry distinct counts too, to prove they stay out of
    /// both. detected = 7 + 4 + 2 = 13, valid = 7 + 3 + 4 + 2 + 9 = 25, score = 1300 / 25 = 52.0.
    #[test]
    fn the_score_is_detected_over_valid_with_uncovered_counted_against_it() {
        ...
        assert_eq!(summary.detected(), 13);
        assert_eq!(summary.valid(), 25);
        assert!(
            (summary.score() - 52.0).abs() < f64::EPSILON,
            "13 detected over 25 valid is 52.0%, not {}",
            summary.score()
        );
    }
}
...
mod removal {
    ...
    fn a_line_that_is_only_a_directive_is_removable() {
        assert!(removable("// gamma::skip(arith)"));
