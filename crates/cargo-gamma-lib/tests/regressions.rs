...
use cargo_gamma_lib::ci::{self, Level};
use cargo_gamma_lib::discover::matches_glob;
use cargo_gamma_lib::estimate;
use cargo_gamma_lib::fix::{Edit, apply};
use cargo_gamma_lib::migrate::translate;
use cargo_gamma_lib::model::{Mutant, Outcome};
use cargo_gamma_lib::ops::collect::{Shape, collect};
use cargo_gamma_lib::ops::registry::Selection;
use cargo_gamma_lib::parse::SourceFile;
use cargo_gamma_lib::suppress::directives;
use common::FakeHost;
use core::time::Duration;
use std::collections::BTreeSet;
use std::fs;

/// The repository root, so a test can read the documents the fixes promised to keep true.
...
fn report_with(shard: Option<(u32, u32)>, started_at: u64, mutants: &[(&str, &str)]) -> cargo_gamma_lib::elements::Report {
...
fn cli(args: &[&str]) -> (i32, String) {
    ...
    (code, format!("{}{}", host.stdout(), host.stderr()))
}
...
fn issue_004_a_stall_does_not_claim_to_have_found_the_hung_test() {
    ...
    let design = document("docs/DESIGN.md");
...
fn issue_005_the_worst_case_pays_for_confirming_every_timeout() {
    ...
    let estimate = estimate::project(&[], Duration::ZERO, budget, Duration::ZERO, Duration::ZERO, 1);
...
fn issue_010_the_internals_feature_is_never_on_for_an_ordinary_build() {
    ...
    // configuration. A feature enabled by the crate's own dev-dependency has the same effect for
    // test builds without depending on the optimization level.
    let manifest = document("crates/cargo-gamma-lib/Cargo.toml");

    assert!(manifest.contains("internals = []"), "the feature is gone: {manifest}");
    assert!(
        manifest.contains(r#"cargo-gamma-lib = { path = ".", features = ["internals"] }"#),
        "the self dev-dependency that enables it for tests is gone"
    );
    assert!(
...
fn issue_023_the_readme_does_not_claim_more_than_the_tool_does() {
    // An unqualified "superset of cargo-mutants" is a promise the tool does not keep, and the way a
    // user finds out is by migrating and losing something.
    let readme = document("README.md");
...
#[test]
fn issue_026_an_untranslatable_config_key_is_explained_not_dropped() {
    // A key that vanishes silently is a setting the user still believes is in force. Every line of
    // a migration has to say what became of it, whether gamma recognises it or not.
    let out = translate("output = \"target/mutants\"\nsome_future_key = 1\n").expect("translates");

    assert!(out.text.contains("output"), "the key vanished: {}", out.text);
    assert!(
        out.settled.iter().any(|line| line.contains("output")),
        "a recognised key was dropped instead of explained: {:?}",
        out.settled
    );

    assert!(out.text.contains("some_future_key"), "the key vanished: {}", out.text);
    assert!(
        out.preserved.iter().any(|line| line.contains("some_future_key")),
        "an unrecognised key was dropped instead of counted: {:?}",
        out.preserved
    );
}
...
fn issue_029_a_failed_run_does_not_keep_its_build_output() {
    ...
    assert!(
        workspace.contains("remove_dir_all(self.target.as_std_path())"),
...
fn issue_011_a_verdict_for_code_that_no_longer_exists_leaves_the_denominator() {
    ...
    let merged = cargo_gamma_lib::merge::merge(&[("older".to_owned(), older), ("newer".to_owned(), newer)], 300, None);
...
fn issue_011_a_shard_does_not_withdraw_what_it_never_looked_at() {
    ...
    let merged = cargo_gamma_lib::merge::merge(&[("first".to_owned(), first), ("second".to_owned(), second)], 300, None);
...
fn issue_011_a_current_population_does_not_blank_the_verdicts_it_is_merged_with() {
    ...
    let merged = cargo_gamma_lib::merge::merge(&[("run".to_owned(), run), ("listing".to_owned(), listing)], 300, None);
...
fn issue_012_code_the_compiler_never_saw_is_never_mutated() {
    ...
    let mut cfgs = cargo_gamma_lib::cfg::CfgSet::parse("unix\ntarget_os=\"linux\"\n");

    cfgs = cfgs.with_features(["lib/other".to_owned()]);

    let mutants = cargo_gamma_lib::ops::collect::collect_in(&source, &Selection::everything(), &cfgs);
...
fn issue_012_an_unmodelled_predicate_leaves_the_code_mutable() {
    ...
    let cfgs = cargo_gamma_lib::cfg::CfgSet::parse("unix\n");
    let mutants = cargo_gamma_lib::ops::collect::collect_in(&source, &Selection::everything(), &cfgs);
...
fn issue_030_the_guard_does_not_allocate() {
    ...
    let source = fs::read_to_string(repository().join("crates/cargo-gamma-rt/src/lib.rs")).expect("the runtime source");
...
fn the_regression_suite_covers_every_issue_that_was_fixed() {
    ...
    let elsewhere: BTreeSet<&str> = [
        "003 exec::subtree",  // process groups and job objects
