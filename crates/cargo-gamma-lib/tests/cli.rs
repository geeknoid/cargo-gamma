...
#[test]
fn a_measured_run_journals_every_testing_verdict_in_the_scratch_directory() {
    let dir = workspace(SUBJECT);
    let (code, host) = invoke(&dir, &["run", "--whole-test-binaries", "--jobs", "1"]);

    assert_eq!(code, EXIT_OK, "{}", host.err());

    let path = scratch_base(&dir).join("testing_progress.log");
    ...
    let record = serde_json::json!({
        ...
        "files": [{
            ...
            "mutants": [
                ...
                { "id": killed, "outcome": "Killed", "killed_by": "tests::ranges_work" },
            ],
        }],
        "hints": {
...
fn promoting_hints_writes_only_what_cannot_move_a_score() {
    ...
    let written = fs::read_to_string(dir.path().join(".cargo/gamma-hints.json")).expect("the artifact should have been written");
    ...
    // The record held a `Killed` verdict for the probed mutant. Carrying it would settle part of a
    // score out of somebody else's run, which is the one thing this file may never do.
    ...
    assert!(host.err().contains("never settle a verdict"), "{}", host.err());
}
...
fn previewing_a_promotion_writes_no_file() {
    ...
    assert!(
        !dir.path().join(".cargo/gamma-hints.json").exists(),
...
fn regenerating_an_unchanged_artifact_changes_no_bytes() {
    ...
    let (first_code, _first) = invoke(&dir, &["hints"]);
    let first = fs::read_to_string(dir.path().join(".cargo/gamma-hints.json")).expect("the artifact exists");

    let (second_code, host) = invoke(&dir, &["hints"]);
    let second = fs::read_to_string(dir.path().join(".cargo/gamma-hints.json")).expect("the artifact still exists");

    assert_eq!(first_code, EXIT_OK);
...
fn promoting_without_a_record_writes_nothing_and_explains_why() {
    ...
    assert!(!dir.path().join(".cargo/gamma-hints.json").exists());
}
...
fn a_corrupt_artifact_costs_time_and_never_signal() {
    ...
    fs::write(dir.path().join(".cargo/gamma-hints.json"), "{ not json at all").expect("could not write the artifact");
    ...
    let written = fs::read_to_string(dir.path().join(".cargo/gamma-hints.json")).expect("the artifact was rewritten");
