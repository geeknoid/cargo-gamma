...
/// The whole of the record, end to end: one run writes it, the next adopts it.
#[test]
fn a_second_run_carries_forward_what_the_first_one_settled() {
    step_aside_if_nested!();
    let dir = workspace(SUBJECT);
    ...
    let record = scratch_base(&dir).join("last-run.json");
    let text = fs::read_to_string(&record).expect("the run record should have been written");

    assert!(text.contains("\"killed\"") || text.contains("Killed"), "{text}");

    // With `--incremental no`, the record is ignored so kills are redone without adopting.
    ...
    // By default (`--incremental full`), the record carries forward settled kills.
    let (second, output) = session(&dir, &["--mutators", "relational"]);

    assert_eq!(second, EXIT_OK, "{output}");
    assert!(output.contains("Iterating"), "{output}");
    assert!(output.contains("the run record"), "{output}");
}

/// A kill is only as good as the test that made it, and no digest of the mutant's own file can
/// tell that: the test lives in another file entirely.
#[test]
fn a_carried_kill_whose_test_was_deleted_is_run_again() {
    step_aside_if_nested!();
    let dir = workspace("pub fn is_adult(age: u32) -> bool {\n    age >= 18\n}\n");
    let integration = dir.path().join("tests/boundary.rs");

    fs::create_dir_all(dir.path().join("tests")).expect("could not create tests");
    fs::write(
        &integration,
    ...
    // An index that found no test at all means the sources could not be read rather than that the
    // suite is gone, and nothing is refused on that evidence — so one test has to outlive the
    // deletion for the deletion to mean anything.
    fs::write(dir.path().join("tests/other.rs"), "#[test]\nfn unrelated() {}\n").expect("could not write the second test");
    ...
    // `src/lib.rs` is untouched, so the record still offers its kill. Only the test that made it is
    // gone, and that is a fact about a file the mutant's digest says nothing about.
    fs::remove_file(&integration).expect("could not delete the integration test");

    let (second, output) = session(&dir, &["--mutators", "relational"]);

    assert_eq!(second, EXIT_OK, "{output}");
    assert!(!output.contains("Iterating"), "a deleted test must not carry its kill: {output}");
}
...
fn a_file_whose_every_mutant_is_unviable_still_converges() {
    ...
    let (code, output) = session(&dir, &["--mutators", "fn_value.some_default", "--unviable"]);
...
fn the_config_file_is_honoured_by_a_real_run() {
    ...
    fs::write(dir.path().join(".cargo/gamma.toml"), "mutators = [\"stmt\"]\nmin-score = 100.0\n").expect("could not write the config");
...
fn a_foreign_config_is_reported_as_unread() {
    ...
    let note = output.find(".cargo/mutants.toml is not read").expect("foreign-config note");
    let preparing = output.find("Preparing").expect("preparation phase");

    assert!(note < preparing, "{output}");
}
...
fn migrating_writes_a_config_the_tool_then_honours() {
    ...
    assert!(host.err().contains("left as TODO"), "{}", host.err());
...
fn a_merged_score_gate_can_fail_the_build() {
    ...
    let (code, output) = session(
        &dir,
        &["--mutators", "stmt", "--json-report", path.to_str().expect("path is not UTF-8")],
    );
...
fn nextest_enumeration_failure_caused_only_by_a_mutant_kills_it() {
    ...
    let report = fs::read_to_string(scratch_base(&dir).join("report.json")).expect("could not read the report");
...
fn a_survivor_reaches_the_diff_and_the_security_tab() {
    ...
    let (code, out, err) = session_on(host, &dir, &["--mutators", "stmt", "--sarif-report", sarif.as_str()]);
...
fn the_diag_flag_stays_out_of_the_help_it_is_not_for_users_of_the_tool() {
    ...
    // `--diag-bundle` also starts with `--diag`, so the bare flag is the one with nothing after it.
    let bare = text.split("--diag").skip(1).any(|rest| !rest.starts_with('-'));

    assert_eq!(code, EXIT_OK, "{text}");

    // The prose dump is a tool-author's instrument. The bundle is a bug reporter's, so it stays.
    assert!(!bare, "{text}");
    assert!(text.contains("--diag-bundle"), "{text}");
...
fn leaking_the_scratch_tree_reports_where_it_was_kept() {
    ...
    let tree = scratch_base(&dir).join("tree");
