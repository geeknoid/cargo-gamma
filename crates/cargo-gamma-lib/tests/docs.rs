...
use cargo_gamma_lib::internals::{config::Config, docs};
use std::fs;

/// The documentation files carrying generated blocks, relative to the repository root.
...
fn the_optimized_campaign_profile_is_valid_and_preserves_development_checks() {
    ...
    assert!(prose.contains("invalidates build and verdict reuse"));
}
...
///
/// The file's whole promise is that copying it to `.cargo/gamma.toml` changes nothing until you
...
/// before it, which reads as deliberate and is how `--min-score` and `--html-report` came to be
...
fn every_crate_that_can_forbid_unsafe_code_does() {
    ...
    let crates = root_dir().join("crates");
        ...
        .flatten()
    {
        ...
        if !path.is_dir() || ALLOWED.contains(&name.as_str()) {
            continue;
        }
