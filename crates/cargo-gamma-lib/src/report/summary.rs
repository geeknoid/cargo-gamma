...
pub fn skipped<H: Host>(host: &mut H, plan: &Plan, styler: Styler) -> Result<()> {
    ...
    writeln!(
        stream,
        "{} {} could not be analyzed and {} left out of the population, so the score below is a claim about the rest of the workspace:",
        styler.warning(),
        quantity(plan.skipped.len(), "file"),
        if plan.skipped.len() == 1 { "was" } else { "were" }
    )?;
...
pub fn summarize<H: Host>(host: &mut H, plan: &Plan, styler: Styler, listings: Listings) -> Result<()> {
    ...
    //
    // A timeout counts as detected, so the only other sign of a hang is a run that took longer than
    // it should have; they are also the mutants most worth acting on, because each is a repeated
    // cost until it is suppressed or fixed. A memory kill is listed for a different reason: it is
    // the outcome most likely to be wrong, since a ceiling set too tight convicts a healthy mutant,
    ...
    //
    // Out-of-memory is named separately from killed for the same reason uncovered is named
    // separately from survived. All three of killed, timed out and out of memory count toward the
    // score, but they ask the reader to do different things: a killed mutant needs nothing, a
...
pub fn session_notes<H: Host>(host: &mut H, session: &Session, hints_missing: bool, styler: Styler) -> Result<()> {
    let mut stream = host.error();
    ...
    if hints_missing {
        writeln!(
            stream,
            "{} no `.cargo/gamma-hints.json` was found; run `cargo gamma hints` to promote ordering hints",
            styler.note("Hints")
        )?;
    }
...
mod tests {
    ...
    fn an_oracle_that_lost_test_targets_says_how_many() {
        ...
        session_notes(&mut host, &narrowed, false, Styler::new(false)).expect("notes");
    ...
    fn notes(widened: bool) -> String {
        ...
        session_notes(&mut host, &session(widened), false, Styler::new(false)).expect("notes");
    ...
    #[test]
    fn a_missing_hints_file_suggests_promoting_one() {
        let mut host = Sink::default();

        session_notes(&mut host, &session(false), true, Styler::new(false)).expect("notes");

        let text = String::from_utf8(host.err).expect("UTF-8");

        assert!(text.contains(".cargo/gamma-hints.json"), "{text}");
    ...
    fn results_the_live_display_already_named_are_not_repeated() {
        ...
        assert!(
            text.contains("3 mutants (1 killed, 1 survived, 1 timed out, 0 out of memory, 0 uncovered => 66.7%)"),
            "{text}"
        );
    }

    #[test]
    fn a_memory_kill_is_listed_and_counted_separately_from_a_killed_mutant() {
        // The distinction is the point: both count toward the score, but a killed mutant needs
    ...
    fn a_run_that_could_not_bound_memory_does_not_say_so_on_the_console() {
        ...
        session_notes(&mut host, &settled, false, Styler::new(false)).expect("notes");
    ...
    fn a_closed_diagnostic_stream_fails_the_session_notes() {
        fails_at_every_line(1, |host| session_notes(host, &session(true), false, Styler::new(false)));
    }

    /// `session_notes` writes up to three separate lines, one for each condition it has something
    ...
    fn a_pipe_that_closes_partway_through_fails_whichever_note_was_writing() {
        ...
        fails_at_every_line(3, |host| session_notes(host, &session, true, Styler::new(false)));
    }
}
