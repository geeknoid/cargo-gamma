...
/// moment time out — and a timeout counts as a detection, so the noise is scored as a kill and the
/// score reads better than the suite deserves.
...
mod tests {
    ...
    fn a_live_run_reports_where_each_phase_spent_its_time() {
        ...
        let text = render(&plan(Vec::new()), Some(&live), 4, Duration::from_secs(60));
