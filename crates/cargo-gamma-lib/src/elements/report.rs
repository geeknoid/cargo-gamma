...
///
/// The schema has no out-of-memory status, so the verdict is exported as `Timeout` and this prefix
/// is the only thing that tells the two apart once a report has been written. `statusReason` is a
/// free-form string in the schema, so carrying the distinction here costs the schema nothing, where
/// a field of our own invention would be an extension a strict validator could reject.
...
///
/// `Timeout` is detected by the schema, matching the score printed here, while suppressions become
/// `Ignored` and leave its denominator. Out-of-memory mutants also map to `Timeout`, the schema's
/// only detected resource-exhaustion status; [`OUT_OF_MEMORY_PREFIX`] preserves the distinction.
...
fn reason_for(mutant: &Mutant) -> Option<String> {
    ...
    match mutant.outcome {
        ...
        Outcome::Timeout => Some(mutant.note.clone().unwrap_or_else(|| "the test run exceeded its budget".to_owned())),
        ...
        // it here would leave the reader a bare `Timeout` with nothing to explain it — the more so
        // because the status is not even the verdict that was reached.
...
fn object<'value>(value: &'value Value, path: &str) -> SchemaResult<&'value serde_json::Map<String, Value>> {
    value.as_object().ok_or_else(|| format!("{path} must be an object"))
}

fn array<'value>(value: &'value Value, path: &str) -> SchemaResult<&'value Vec<Value>> {
    value.as_array().ok_or_else(|| format!("{path} must be an array"))
}

fn required<'value>(object: &'value serde_json::Map<String, Value>, name: &str, path: &str) -> SchemaResult<&'value Value> {
    object.get(name).ok_or_else(|| format!("{path} is missing required field `{name}`"))
}

fn string<'value>(value: &'value Value, path: &str) -> SchemaResult<&'value str> {
    value.as_str().ok_or_else(|| format!("{path} must be a string"))
}

fn number(value: &Value, path: &str) -> SchemaResult<()> {
    if value.is_number() {
        Ok(())
    } else {
...
fn boolean(value: &Value, path: &str) -> SchemaResult<()> {
    if value.is_boolean() {
        Ok(())
    } else {
...
fn integer(value: &Value, path: &str) -> SchemaResult<u64> {
...
fn validate_file(value: &Value, path: &str) -> SchemaResult<()> {
    ...
    let _ = string(required(file, "language", path)?, &format!("{path}.language"))?;
    let _ = string(required(file, "source", path)?, &format!("{path}.source"))?;
...
fn validate_mutants(value: &Value, path: &str) -> SchemaResult<()> {
    ...
    for (index, mutant) in mutants.iter().enumerate() {
        ...
        let encoded = serde_json::to_vec(mutant).map_err(|cause| format!("{path} could not be compared: {cause}"))?;
...
fn validate_mutant(value: &Value, path: &str) -> SchemaResult<()> {
    ...
    let id = string(required(mutant, "id", path)?, &format!("{path}.id"))?;
    let _ = string(required(mutant, "mutatorName", path)?, &format!("{path}.mutatorName"))?;
    validate_location(required(mutant, "location", path)?, &format!("{path}.location"), false)?;

    let status_path = format!("{path}.status");
    let status = string(required(mutant, "status", path)?, &status_path)?;
    ...
    for name in ["description", "replacement", "statusReason"] {
        if let Some(value) = mutant.get(name) {
            let _ = string(value, &format!("{path}.{name}"))?;
        }
    }

    for name in ["duration", "testsCompleted"] {
        if let Some(value) = mutant.get(name) {
            number(value, &format!("{path}.{name}"))?;
        }
    }

    for name in ["coveredBy", "killedBy"] {
        if let Some(value) = mutant.get(name) {
            validate_strings(value, &format!("{path}.{name}"))?;
        }
    }

    if let Some(value) = mutant.get("static") {
        boolean(value, &format!("{path}.static"))?;
    }

    Ok(())
}

fn validate_strings(value: &Value, path: &str) -> SchemaResult<()> {
    for (index, value) in array(value, path)?.iter().enumerate() {
        let _ = string(value, &format!("{path}[{index}]"))?;
    }

    Ok(())
}

fn validate_location(value: &Value, path: &str, open_end: bool) -> SchemaResult<()> {
    let location = object(value, path)?;
    validate_position(required(location, "start", path)?, &format!("{path}.start"))?;

    if open_end {
        if let Some(end) = location.get("end") {
            validate_position(end, &format!("{path}.end"))?;
        }
    } else {
        validate_position(required(location, "end", path)?, &format!("{path}.end"))?;
    }

    Ok(())
}

fn validate_position(value: &Value, path: &str) -> SchemaResult<()> {
    let position = object(value, path)?;

    for name in ["line", "column"] {
        let field = format!("{path}.{name}");

        if integer(required(position, name, path)?, &field)? == 0 {
...
fn validate_test_files(value: &Value, path: &str) -> SchemaResult<()> {
    for (name, file) in object(value, path)? {
        ...
        if let Some(source) = file.get("source") {
            let _ = string(source, &format!("{file_path}.source"))?;
        }

        for (index, test) in array(required(file, "tests", &file_path)?, &format!("{file_path}.tests"))?
            .iter()
            .enumerate()
        {
            ...
            let _ = string(required(test, "id", &test_path)?, &format!("{test_path}.id"))?;
            let _ = string(required(test, "name", &test_path)?, &format!("{test_path}.name"))?;

            if let Some(location) = test.get("location") {
                validate_location(location, &format!("{test_path}.location"), true)?;
            }
        }
    }
...
fn validate_performance(value: &Value, path: &str) -> SchemaResult<()> {
    ...
    for name in ["setup", "initialRun", "mutation"] {
        number(required(performance, name, path)?, &format!("{path}.{name}"))?;
    }
...
fn validate_framework(value: &Value, path: &str) -> SchemaResult<()> {
    ...
    let _ = string(required(framework, "name", path)?, &format!("{path}.name"))?;

    if let Some(version) = framework.get("version") {
        let _ = string(version, &format!("{path}.version"))?;
    }
    ...
    if let Some(branding) = framework.get("branding") {
        ...
        let homepage = string(
            ...
            &format!("{branding_path}.homepageUrl"),
        )?;
        ...
        if let Some(image) = branding.get("imageUrl") {
            let _ = string(image, &format!("{branding_path}.imageUrl"))?;
        }
    }
    ...
    if let Some(dependencies) = framework.get("dependencies") {
        ...
        for (name, version) in object(dependencies, &dependencies_path)? {
            let _ = string(version, &format!("{dependencies_path}[{name:?}]"))?;
        }
    }
...
fn validate_system(value: &Value, path: &str) -> SchemaResult<()> {
    ...
    boolean(required(system, "ci", path)?, &format!("{path}.ci"))?;

    if let Some(os) = system.get("os") {
        let os_path = format!("{path}.os");
        let os = object(os, &os_path)?;
        let _ = string(required(os, "platform", &os_path)?, &format!("{os_path}.platform"))?;

        for name in ["description", "version"] {
            if let Some(value) = os.get(name) {
                let _ = string(value, &format!("{os_path}.{name}"))?;
            }
        }
    }
    ...
    if let Some(cpu) = system.get("cpu") {
        ...
        number(required(cpu, "logicalCores", &cpu_path)?, &format!("{cpu_path}.logicalCores"))?;

        if let Some(clock) = cpu.get("baseClock") {
            number(clock, &format!("{cpu_path}.baseClock"))?;
        }

        if let Some(model) = cpu.get("model") {
            let _ = string(model, &format!("{cpu_path}.model"))?;
        }
    }
    ...
    if let Some(ram) = system.get("ram") {
        ...
        number(required(ram, "total", &ram_path)?, &format!("{ram_path}.total"))?;
    }
...
mod tests {
    ...
    fn unviable_and_timeout_mutants_explain_their_status() {
        ...
        assert_eq!(reason_for(&timed_out), Some("the test run exceeded its budget".to_owned()));
    ...
    fn an_out_of_memory_mutant_keeps_its_note_in_the_status_reason() {
        // The status the schema forces on this verdict is `Timeout`, so the reason is the only
        // place the reader learns that the ceiling, not the clock, is what stopped it.
