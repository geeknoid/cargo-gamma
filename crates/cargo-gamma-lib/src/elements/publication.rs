...
///
/// On Unix the lock belongs to the destination directory, whose inode survives a replacement of
/// any file in it. Other platforms use a persistent sibling lock file: removing a lock file on
/// drop lets a waiter retain its old inode while a later writer creates and locks a new one,
/// splitting one lock domain into two.
///
...
pub(crate) fn write_if_unchanged(path: &Utf8Path, expected: Option<&str>, contents: &str) -> Result<Publication> {
    let destination = crate::paths::physical(path)?;
    create_parents(&destination)?;
...
pub(crate) fn remove_if_unchanged(path: &Utf8Path, expected: &str) -> Result<Publication> {
    let destination = crate::paths::physical(path)?;
    let _lock = lock(&destination)?;
...
/// in its own right, because this is the path that prints "Wrote `.cargo/gamma.toml`" and a file
...
/// Locks the directory that anchors `path` for cooperating conditional writers.
///
/// An editor that does not take this lock is caught only when it saves before the final content
/// comparison. The lock supplies the cooperative protocol: a hints rollback and another hints
/// promotion cannot both decide that the same generation is current and then replace one another.
#[cfg(unix)]
fn lock(path: &Utf8Path) -> Result<File> {
    let parent = path
        .parent()
        .filter(|parent| !parent.as_str().is_empty())
        .unwrap_or_else(|| Utf8Path::new("."));
    let file =
        File::open(parent.as_std_path()).map_err(|cause| error!("could not open publication directory `{parent}`").caused_by(cause))?;

    file.lock()
        .map_err(|cause| error!("could not lock publication directory `{parent}`").caused_by(cause))?;

    Ok(file)
}

/// Locks a stable sibling on platforms where a directory cannot be locked.
#[cfg(not(unix))]
fn lock(path: &Utf8Path) -> Result<File> {
    ...
    let file = OpenOptions::new()
        ...
        .map_err(|cause| error!("could not open publication lock `{lock}`").caused_by(cause))?;

    file.lock()
        .map_err(|cause| error!("could not lock publication lock `{lock}`").caused_by(cause))?;

    Ok(file)
}

/// The stable sibling lock name for one resolved destination.
#[cfg(not(unix))]
fn lock_path(path: &Utf8Path) -> Utf8PathBuf {
    let name = path.file_name().unwrap_or("report");

    path.with_file_name(format!(".{name}.cargo-gamma.lock"))
}
...
/// synced. For a report the loss is invisible — the reader sees the previous file or none — but
/// `migrate` prints "Wrote .cargo/gamma.toml", and a success the user was told about and cannot
/// find afterwards is a different kind of failure.
///
...
mod tests {
    ...
    fn a_conditional_replacement_reports_post_rename_sync_failure_as_published() {
        ...
        let publication = write_if_unchanged(&path, Some("before"), "after").expect("the rename itself succeeds");
    ...
    fn a_conditional_removal_reports_post_unlink_sync_failure_as_published() {
        ...
        let publication = remove_if_unchanged(&path, "before").expect("the removal itself succeeds");
    ...
    fn conditional_publication_has_a_deterministic_external_editor_boundary() {
        ...
        let conflict = write_if_unchanged(&path, Some("before"), "gamma").expect("comparison");
        ...
        let published = write_if_unchanged(&path, Some("before"), "gamma").expect("publication");
