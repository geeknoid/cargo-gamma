...
fn write_glossary(out: &mut String, layout: Layout) {
    ...
    for (term, meaning) in [
        ...
        (
            "Timed out",
            "the suite never finished with the mutant active. Counted as detected: the change did not go unnoticed.",
        ),
        (
            "Uncovered",
...
mod tests {
    ...
    /// easily left without a row at all — a memory kill, a flake, and a mutant the build never
    /// compiled. With those at zero the table cannot be caught omitting them: a run whose mutants
    /// were all stopped by a memory ceiling would render a header, a separator and a perfect score
    /// with nothing between them, and an all-zero fixture reproduces none of that. Summing the rows
    ...
    fn every_verdict_in_the_population_is_accounted_for_in_the_run_table() {
        ...
        // The score's numerator is on the page too: killed, timed out and stopped by the memory
        // limit, which is the row most easily left out while its mutants stay in the score.
        let detected: u32 = rows
            .iter()
            .filter(|(label, _, _)| matches!(label.as_str(), "Killed" | "Timed out" | "Stopped by the memory limit"))
            .map(|(_, count, _)| count)
            .sum();
    ...
    /// A run whose every mutant was stopped by its memory ceiling scores 100%, and the table under
    /// that score has to say what earned it rather than being empty.
    #[test]
    fn a_run_of_nothing_but_memory_kills_still_has_a_breakdown() {
        let summary = Summary {
            out_of_memory: 3,
            ..Summary::default()
        };
    ...
    fn unscored_outcomes_are_named_as_not_scored_in_the_run_table() {
        ...
        // table must not present them as a share of the mutation score — while the memory kill
        // beside them, which is in the numerator, must take its share like any other detection.
