...
/// The directory the artifact lives in, relative to the workspace root.
///
/// `.cargo` because that is already where this tool's configuration lives — `.cargo/gamma.toml` —
/// so a workspace adopting gamma acquires one directory rather than two, and a reader who finds one
/// file there is not surprised by the other.
const DIRECTORY: &str = ".cargo";
...
impl Hints {
    ...
    pub fn write(&self, path: &Utf8Path) -> Result<Promotion> {
        ...
        match crate::elements::write_if_unchanged(path, before.as_deref(), &text)
        ...
        match Self::verified(path, self) {
            ...
            Err(cause) => Err(restored(path, before.as_deref(), &text, cause)),
        }
    }
...
fn restored(path: &Utf8Path, before: Option<&str>, published: &str, cause: crate::error::Error) -> crate::error::Error {
    let restored = before.map_or_else(
        || crate::elements::remove_if_unchanged(path, published),
        |text| crate::elements::write_if_unchanged(path, Some(published), text),
    );
...
mod tests {
    ...
    fn a_corrupt_artifact_is_no_hints_at_all() {
        ...
        fs::create_dir_all(root.join(DIRECTORY)).expect("the directory should be creatable");
