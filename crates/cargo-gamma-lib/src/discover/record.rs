//! What the last run established about each mutant, so the next one does not rediscover it.
//!
//! One record, one invalidation rule *per tier*, three levels of trust. See *What a run remembers*
//! in `docs/DESIGN.md` for the lifetimes this sits in the middle of.
...
/// How much of a record a run is willing to believe.
///
/// The distinction the record exists to keep is not *what* was learned but *what believing it
/// costs*. Unviability moves no verdict — an unviable mutant is out of the score either way — so
/// adopting it can only save time. A kill is a claim about the test suite, and adopting one
/// silently would let a score be built out of knowledge nobody chose. So `Trust::Free` (used in
/// `build` incremental mode) is for unviability and `Trust::Settled` (used in `full` incremental
/// mode) carries verified verdicts.
...
pub enum Trust {
    ...
    /// Everything the record settled, kills included.
    Settled,
}
...
pub enum Tier {
    ...
    /// The verdicts a rerun would learn nothing from: kills and deliberate skips.
    ///
    /// Adopted under `--incremental full`, and revalidated against every captured workspace input.
    /// The compiler remains required: compiler and standard-library changes can alter a test's
    /// behavior, and a false carried kill raises the score. `Profile` is another obvious axis:
    /// a `debug_assert!` catches a mutant in one profile and does not exist in the other.
    Verdict,
...
/// Whether a verdict is one a rerun would learn nothing from.
///
/// The model-side twin of `elements::settled_verdict`, and it must agree with it: a mutant the
/// build gave up on, a flake, and a mutant stopped by its memory ceiling all established nothing
/// about the tests, so none of them is recorded.
const fn settled_verdict(outcome: Outcome) -> bool {
    matches!(outcome, Outcome::Killed | Outcome::CompileError | Outcome::Ignored)
}
...
impl RunRecord {
    ...
    pub(crate) fn settled_against(
        ...
        for file in &self.files {
            ...
            // Length first: it is a `stat` against a read, and a file whose length has changed
            // cannot be the one that was hashed however the bytes fell.
            ...
            for entry in &file.mutants {
                ...
                if entry.outcome == Outcome::Killed && !still_killed(entry, killers, &files_by_path, root) {
                    declined += 1;
    ...
    pub fn store(&self, base: &Utf8Path, root: &Utf8Path) {
        if !self.inputs.matches_current(root) {
            return;
        }

        let earlier = Self::load_raw(base).unwrap_or_default();
        let merged = self.absorbing(&earlier, root);
    ...
    fn absorbing(&self, earlier: &Self, root: &Utf8Path) -> Self {
        ...
        for file in &self.files {
            ...
            if !file.mutants.is_empty() {
                ...
                if !earlier_file.mutants.is_empty() && is_unchanged(earlier_file, root) {
        ...
        for file in &carried {
            if !seen.contains(&file.path) && is_unchanged(file, root) {
                files.push(file.clone());
            }
        }
...
///
/// The same rule `--incremental full` applies to a record, and for the same reason: a mutant's identity says
/// nothing about the tests, so carrying a kill whose test was deleted claims coverage that no
/// longer exists — silently, and in the one direction a tool built to say a suite is weaker than it
/// looks must never be wrong in.
///
fn still_killed(entry: &Entry, killers: &Killers, files_by_path: &HashMap<&Utf8Path, &RecordedFile>, root: &Utf8Path) -> bool {
    ...
    killers.file_digest(test_file_path) == Some(recorded.digest.as_str()) && is_unchanged(recorded, root)
}

/// Whether a recorded file is still on disk exactly as it was when its mutants were judged.
///
/// The same question [`RunRecord::settled`] asks of every entry it reads, asked here so that an
/// entry nobody would believe is not carried forward. Length first for the same reason: it is a
/// `stat` against a read.
fn is_unchanged(file: &RecordedFile, root: &Utf8Path) -> bool {
    let path = root.join(&file.path);

    if !fs::metadata(path.as_std_path()).is_ok_and(|metadata| metadata.len() == file.size) {
        return false;
    }

    fs::read(path.as_std_path()).is_ok_and(|bytes| digest(&bytes) == file.digest)
}
...
///
/// Taken as values rather than read here so that a test can vary them: the workspace forbids
/// writing the process environment — see `docs/DESIGN.md` — which leaves no other way to state what
...
///
/// The workspace forbids writing the process environment — see `docs/DESIGN.md` — so the variable
/// is taken as a value rather than looked up where it is needed. Everything else the digest covers
/// arrives through [`Context`] or through the workspace root.
...
mod tests {
    ...
    /// The whole point of the two tiers: a kill settles part of the score, so nobody gets one
    /// without asking.
    #[test]
    fn a_kill_is_carried_only_for_a_run_that_asked_for_it() {
        let (_dir, root) = workspace("record-trust-", "fn add() {}");
        let index = killers(&root, "#[test]\nfn caught() {}\n");
        let record = from_run(&root, &[killed("abc", Some("caught"))], &envelope());

        assert!(
            record.settled(&root, Trust::Free, &index, &envelope()).0.is_empty(),
            "a kill is not free"
        );
        assert_eq!(
            record.settled(&root, Trust::Settled, &index, &envelope()).0.get("abc"),
            Some(&Outcome::Killed)
        );
    }

    /// A file digest cannot catch this: the mutant's file is untouched and the test that caught it
    /// lives somewhere else entirely.
    #[test]
    fn a_kill_whose_test_no_longer_exists_is_refused_and_counted() {
        let (_dir, root) = workspace("record-gone-", "fn add() {}");
        let index = killers(&root, "#[test]\nfn survivor() {}\n");
        let record = from_run(&root, &[killed("abc", Some("deleted"))], &envelope());
        let (settled, declined) = record.settled(&root, Trust::Settled, &index, &envelope());

        assert!(settled.is_empty(), "the kill must not be carried");
        assert_eq!(declined, 1);
    }

    /// A kill with nobody's name on it cannot be revalidated, so it is treated exactly as one whose
    /// test is gone.
    #[test]
    fn a_kill_that_names_no_test_is_refused_as_well() {
        let (_dir, root) = workspace("record-nameless-", "fn add() {}");
        let index = killers(&root, "#[test]\nfn caught() {}\n");
        let record = from_run(&root, &[killed("abc", None)], &envelope());
        let (settled, declined) = record.settled(&root, Trust::Settled, &index, &envelope());
    ...
    /// A completed zero-test scan proves a recorded killer no longer exists.
    #[test]
    fn a_completed_zero_test_scan_refuses_a_kill() {
        let (_dir, root) = workspace("record-blind-", "fn add() {}");
        let _initial = killers(&root, "#[test]\nfn caught() {}\n");
        let record = from_run(&root, &[killed("abc", Some("caught"))], &envelope());
        let (settled, declined) = record.settled(&root, Trust::Settled, &Killers::scan(&[]), &envelope());

        assert!(settled.is_empty());
        assert_eq!(declined, 1);
    }

    /// A verdict that established nothing about the tests must not come back as one that did.
    #[test]
    fn only_verdicts_a_rerun_would_learn_nothing_from_are_recorded() {
        ...
        assert_eq!(settled.get("slow"), None);
        assert_eq!(settled.get("skipped"), Some(&Outcome::Ignored));
    }

    /// The killer's name is the only thing that makes a carried kill checkable, so it has to survive
    /// being written and read back.
    #[test]
    fn the_killer_survives_a_round_trip_through_the_file() {
        let (_dir, root) = workspace("record-roundtrip-", "fn add() {}");
        let index = killers(&root, "#[test]\nfn caught() {}\n");

        from_run(&root, &[killed("abc", Some("caught"))], &envelope()).store(&root, &root);

        let (settled, declined) = RunRecord::load(&root).settled(&root, Trust::Settled, &index, &envelope());

        assert_eq!(settled.get("abc"), Some(&Outcome::Killed));
        assert_eq!(declined, 0);
    }

    #[test]
    fn a_qualified_killer_is_not_carried_by_a_same_leaf_collision() {
        let (_dir, root) = workspace("record-killer-collision-", "fn add() {}");
        let _initial = killers(&root, "mod original { #[test] fn catches() {} }\n");
        let record = from_run(&root, &[killed("abc", Some("original::catches"))], &envelope());
        let (_other_dir, other_root) = workspace("record-killer-collision-other-", "fn irrelevant() {}");
        let other = other_root.join("tests.rs");

        fs::write(&other, "mod replacement { #[test] fn catches() {} }\n").expect("replacement test");
        let current = Killers::scan(&[other]);
        let (settled, declined) = record.settled(&root, Trust::Settled, &current, &envelope());
    ...
    #[test]
    fn a_killer_moved_to_another_file_is_not_carried() {
        let (_dir, root) = workspace("record-killer-moved-", "fn add() {}");
        let _initial = killers(&root, "mod tests { #[test] fn catches() {} }\n");
        let record = from_run(&root, &[killed("abc", Some("tests::catches"))], &envelope());
        let (_other_dir, other_root) = workspace("record-killer-moved-other-", "fn irrelevant() {}");
        let other = other_root.join("tests.rs");

        fs::write(&other, "mod tests { #[test] fn catches() {} }\n").expect("moved test");
        let current = Killers::scan(&[other]);
        let (settled, declined) = record.settled(&root, Trust::Settled, &current, &envelope());
    ...
    fn a_narrow_run_writes_back_safe_unviability_after_an_unrelated_change() {
        ...
        let merged = current.absorbing(&earlier, &root);
    ...
    #[test]
    fn a_changed_inherited_test_environment_refuses_a_carried_verdict() {
        let (_dir, root) = workspace("record-environment-", "fn add() {}");
        let index = killers(&root, "#[test]\nfn caught() {}\n");
        let before =
            context_in(&plain(), None, &[(b"SUBJECT_TEST_MODE".to_vec(), b"before".to_vec())]).expect("a named toolchain gives a context");
        let after =
            context_in(&plain(), None, &[(b"SUBJECT_TEST_MODE".to_vec(), b"after".to_vec())]).expect("a named toolchain gives a context");
        let record = from_run(&root, &[killed("abc", Some("caught"))], &before);

        assert_eq!(
            record.settled(&root, Trust::Settled, &index, &before).0.get("abc"),
            Some(&Outcome::Killed),
            "the unchanged inherited environment may reuse its verdict"
        );
        assert!(
            record.settled(&root, Trust::Settled, &index, &after).0.is_empty(),
            "a test whose inherited environment changed must be run again"
        );
    }
