...
use super::{Diff, Plan, TargetFile};
use crate::Result;
...
///
/// Commands that have merged `.cargo/gamma.toml` into their run arguments pass the same options
    ...
    pub(crate) fn for_build_with_cache_inputs(
        ...
        for package in metadata.workspace_packages() {
            ...
            for target in &package.targets {
                ...
                if mutating {
                    roots.entry(package.name.clone()).or_default().push(source_root.to_owned());
                }
                ...
                for absolute in walk_rust_files(directory)? {
                    ...
                    if declaration_seen.insert(absolute.clone()) {
                        declaration_files.entry(package.name.clone()).or_default().push(absolute.clone());
                    }
...
fn mutate(file: &TargetFile, source: &SourceFile, selection: &Selection, cfgs: &Cfgs, defaults: &collect::Defaults) -> Result<Parsed> {
    ...
    collect::check_stated(source)?;

    let candidates = collect::collect_with(source, selection, cfg, defaults);
...
fn reachable(metadata: &Metadata) -> HashMap<String, HashSet<String>> {
    ...
    let mut reach: HashMap<String, HashSet<String>> = HashMap::default();
    ...
    for start in 0..member_count {
        ...
        // does *not* reach a package is exactly what the missing edges make impossible — and so
        // does anything that reaches it, for the same reason and by the same missing edges.
        let reachable_set: HashSet<String> = if hits_opaque {
            member_packages.iter().map(|n| (*n).to_owned()).collect()
        } else {
            seen.iter()
                .enumerate()
                .filter(|(_id, reached)| **reached)
                .map(|(id, _)| member_packages[id].to_owned())
                .collect()
        };

        let _previous = reach.insert(member_packages[start].to_owned(), reachable_set);
    }
...
fn selected_packages(metadata: &Metadata, args: &SelectArgs) -> HashSet<String> {
    let members = || metadata.workspace_packages().iter().map(|package| package.name.clone()).collect();
...
fn owning_package(metadata: &Metadata, dir: &Utf8Path) -> Option<String> {
    ...
    for package in metadata.workspace_packages() {
        ...
        if best.as_ref().is_none_or(|(deepest, _name)| depth > *deepest) {
            best = Some((depth, package.name.clone()));
        }
    }
...
mod tests {
    ...
    fn discovery_propagates_a_configuration_failure() {
        let (_directory, root) = workspace();
        let config = root.join(".cargo/gamma.toml");
    ...
    fn the_profile_decides_which_half_of_a_debug_assertions_gate_is_surveyed() {
        ...
        let debug = mutated_items(&root, &CargoOptions::default());
        let release = mutated_items(
            &root,
            &CargoOptions {
