...
fn capture_tree(root: &Utf8Path, excluded: &[Utf8PathBuf], complete: &mut bool) -> Vec<SnapshotFile> {
    ...
    let mut walker = WalkDir::new(root.as_std_path()).follow_links(false).into_iter();
    ...
    while let Some(entry) = walker.next() {
        ...
        if entry.file_type().is_symlink() {
            ...
            // outcome when it happens. Dangling links have no referent bytes yet; their spelling
            // and own timestamp are the complete input until one appears.
            if let Some(boundary) = &boundary {
                match fs::canonicalize(path.as_std_path()) {
                    ...
                    Err(_unresolved) => *complete = false,
                }
            }
        ...
        files.push(SnapshotFile {
            ...
            digest: digest(&bytes),
            size: bytes.len() as u64,
            // Read after the bytes rather than before them, so that a write landing between the
            // two leaves a time ahead of the content it describes rather than behind it. The stale
            // direction would be dangerous: a recorded time older than the bytes recorded beside
            // it would match a later capture of those same bytes.
