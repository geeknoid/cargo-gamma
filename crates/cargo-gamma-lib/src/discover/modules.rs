//! Working out which files a crate reaches only through `#[cfg(test)]`.
//!
...
//! each file is reached. A file reached only through a `#[cfg(test)]` declaration is test code no
...
pub struct Declaration {
    ...
    /// Whether it, or a module enclosing it, is behind `#[cfg(test)]`.
    pub test_only: bool,
}
...
/// to where it appears.
#[must_use]
pub fn declarations(path: &Utf8Path, ast: &syn::File) -> Vec<Declaration> {
    let mut found = Vec::new();
    let (Some(directory), Some(beside)) = (owned_directory(path), path.parent()) else {
        return found;
    };

    walk(&ast.items, &directory, beside, false, &mut found);
...
fn walk(items: &[Item], directory: &Utf8Path, base: &Utf8Path, inherited: bool, found: &mut Vec<Declaration>) {
    for item in items {
        let Item::Mod(module) = item else { continue };
        let test_only = inherited || is_test_gated(&module.attrs);
        ...
        if let Some((_brace, items)) = module.content.as_ref() {
            let nested = path_attribute(&module.attrs).map_or_else(
            ...
            walk(items, &nested, &nested, test_only, found);
            continue;
        }
        ...
        for target in candidates {
            if target.as_std_path().is_file() {
                found.push(Declaration { target, test_only });
                break;
            }
        }
    }
...
/// Returns whether an item is behind `#[cfg(test)]`.
...
/// Works out which files are reachable only as test code.
///
/// `roots` are the crate roots — a package's lib and bin entry points — which are never test code.
/// A file reachable from one of them without passing through a `#[cfg(test)]` declaration is real
/// code, whatever else also points at it; everything the declarations reach that is not in that set
...
#[must_use]
pub fn test_only(roots: &[Utf8PathBuf], declared: &[(Utf8PathBuf, Vec<Declaration>)]) -> HashSet<Utf8PathBuf> {
    let edges: crate::HashMap<&Utf8Path, &[Declaration]> = declared
        .iter()
        .map(|(from, list)| (from.as_path(), list.as_slice()))
        .collect();
    ...
    // Everything below a test-only module is test code too, so the exclusion follows every edge
    // from there rather than stopping at the file the declaration named.
...
mod tests {
    ...
    fn a_file_stem_that_is_not_a_module_root_owns_a_subdirectory() {
        ...
        assert_eq!(
            owned_directory(Utf8Path::new("/a/src/de.rs")),
            Some(Utf8PathBuf::from("/a/src/de"))
        );
    }
    ...
    fn a_cfg_test_declaration_is_recognised() {
        ...
        let gated: Vec<bool> = ast
            ...
            .filter_map(|item| match item {
                Item::Mod(module) => Some(is_test_gated(&module.attrs)),
                _ => None,
            })
            .collect();
    ...
    fn a_path_attribute_is_read() {
        ...
        let Item::Mod(module) = &ast.items[0] else { panic!("expected a module") };
    ...
    fn a_path_attribute_resolves_beside_the_file_that_wrote_it() {
        ...
        let found = declarations(&declaring, &ast);

        assert_eq!(found.len(), 1, "{found:?}");
        assert_eq!(found[0].target, target);
        assert!(found[0].test_only);
    }
    ...
    fn a_plain_declaration_resolves_in_the_directory_the_file_owns() {
        ...
        let found = declarations(&declaring, &parse("mod raw;"));
    ...
    fn a_file_reached_only_through_a_test_module_is_excluded() {
        ...
        let declared = vec![(
            root.clone(),
            vec![
                Declaration { target: helper.clone(), test_only: false },
                Declaration { target: tests.clone(), test_only: true },
            ],
        )];

        let excluded = test_only(&[root], &declared);
    ...
    fn a_file_a_test_module_shares_with_real_code_is_kept() {
        ...
        let declared = vec![(
            root.clone(),
            vec![
                Declaration { target: shared.clone(), test_only: true },
                Declaration { target: shared, test_only: false },
            ],
        )];

        assert!(test_only(&[root], &declared).is_empty());
    }
    ...
    fn a_file_nothing_declares_is_left_alone() {
        ...
        assert!(test_only(&[root], &[]).is_empty());
    }
    ...
    fn a_module_below_a_test_module_is_excluded_too() {
        ...
        let declared = vec![
            (root.clone(), vec![Declaration { target: outer.clone(), test_only: true }]),
            (outer.clone(), vec![Declaration { target: inner.clone(), test_only: false }]),
        ];
    ...
    fn a_file_declared_by_two_live_modules_is_walked_once() {
        ...
        let declared = vec![
            (
                root.clone(),
                vec![
                    Declaration { target: left.clone(), test_only: false },
                    Declaration { target: right.clone(), test_only: false },
                ],
            ),
            (left, vec![Declaration { target: shared.clone(), test_only: false }]),
            (right, vec![Declaration { target: shared.clone(), test_only: false }]),
        ];
    ...
    fn a_cycle_between_two_files_terminates() {
        ...
        let declared = vec![
            (root.clone(), vec![Declaration { target: first.clone(), test_only: true }]),
            (first.clone(), vec![Declaration { target: second.clone(), test_only: false }]),
            (second.clone(), vec![Declaration { target: first.clone(), test_only: false }]),
        ];
    ...
    fn a_path_with_no_directory_declares_nothing() {
        ...
        assert!(declarations(Utf8Path::new(""), &parse("mod raw;")).is_empty());
    }
    ...
    fn a_declaration_pointing_at_no_file_is_dropped() {
        ...
        assert!(declarations(&root.join("lib.rs"), &parse("mod nowhere;")).is_empty());
    }
    ...
    fn an_attribute_that_is_neither_cfg_nor_path_is_ignored() {
        ...
        assert!(!is_test_gated(&module.attrs));
    }
    ...
    fn a_path_attribute_on_an_inline_module_redirects_everything_below_it() {
        ...
        let found = declarations(&root.join("lib.rs"), &parse(source));
