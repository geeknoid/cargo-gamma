...
#[test]
fn the_bare_comment_spelling_suppresses_exactly_as_the_bracketed_one_does() {
    // The README teaches both spellings; they have to mean the same thing.
    let bare = "// gamma::skip(arith)\nfn f(a: i32, b: i32) -> i32 { a + b }";
    let bracketed = "// #[gamma::skip(arith)]\nfn f(a: i32, b: i32) -> i32 { a + b }";

    for source in [bare, bracketed] {
        let (parsed, mut mutants) = mutants_of(source, "arith");
        let found = directives(&parsed).unwrap();

        assert_eq!(suppress(&mut mutants, &found), mutants.len(), "{source}");
        assert!(!mutants.is_empty());
        assert!(mutants.iter().all(|mutant| mutant.outcome == Outcome::Ignored), "{source}");
        assert_eq!(found[0].channel, Channel::Comment);
    }
}

#[test]
fn a_bare_comment_directive_carries_its_reason() {
    let source = "// gamma::skip(arith, reason = \"fixed point\")\nfn f(a: i32) -> i32 { a + 1 }";
    let (parsed, mut mutants) = mutants_of(source, "arith");
    let found = directives(&parsed).unwrap();
    let _ = suppress(&mut mutants, &found);

    assert_eq!(mutants[0].suppression.as_ref().unwrap().reason.as_deref(), Some("fixed point"));
}

#[test]
fn a_misspelled_bare_comment_directive_is_an_error_rather_than_silence() {
