...
pub enum Command {
    ...
    /// Translate a foreign mutation-testing project to cargo-gamma.
    Migrate(MigrateArgs),
...
pub struct HintsArgs {
    ...
    /// Read the run record from here instead of the workspace's `target`.
    ///
    ...
    #[arg(long, value_name = "PATH", help_heading = "Scratch tree")]
...
pub struct MigrateArgs {
    ...
    #[arg(short = 'd', long, value_name = "PATH", default_value = ".", help_heading = "Migrating")]
    pub dir: Utf8PathBuf,
    ...
    /// Translate a foreign command line instead of a configuration file, printing POSIX-shell-safe words.
    ///
    ...
    #[arg(long, value_name = "ARG", num_args = 1.., allow_hyphen_values = true, conflicts_with = "dry_run", help_heading = "Migrating")]
...
pub struct SelectArgs {
    ...
    /// Only mutate these packages. Defaults to the package the current directory belongs to.
...
pub struct MeasureArgs {
    ...
    ///
    /// Separate from `--package`, which chooses what to mutate. The default oracle is the tests
    /// cargo itself would run here, so this is how a mutant gets judged by a suite living somewhere
    /// else — a workspace that keeps its integration tests in a package of their own wants naming
    /// that package here.
    ...
    ///
    /// The default is capped at the tests `cargo test` would run here, so a mutant in a library is
    /// judged by that library's own suite and not by its dependents'. This lifts the cap and asks
    /// the whole workspace instead, which is more thorough and costs a workspace-wide build and
    /// test run per mutant.
    ...
    /// gives concurrent runs somewhere separate to work. It is refused when relocating would hide
    /// VCS metadata a build script can see from the source checkout. Build artifacts live here too,
    /// so reusing one directory across runs keeps them incremental while a fresh one starts cold.
    #[arg(long, value_name = "PATH", help_heading = "Scratch tree")]
    pub scratch_dir: Option<Utf8PathBuf>,

    /// Copy files version control ignores into the scratch tree as well.
    ///
    ...
    #[arg(long, help_heading = "Scratch tree")]
...
pub struct RunArgs {
    ...
    /// How much an incremental run reuses from the last run: `no` (cold run), `build` (compiler
    /// unviability cache only), `full` (compiler and test verdicts, validated by workspace snapshots).
    ...
    /// Write the self-contained HTML report somewhere other than the default.
    ///
    /// Every run writes one; this only moves it. The page embeds the viewer and the results, so it
    /// opens from a CI artifact, a file share or an air-gapped machine with no network access.
    #[arg(long, value_name = "PATH", help_heading = "Reporting")]
    pub html_report: Option<Utf8PathBuf>,

    /// Write the `mutation-testing-elements` JSON report somewhere other than the default.
    ///
    /// Every run writes one; this only moves it. This is the interchange format the standard
    /// mutation report viewers consume, so it feeds the Azure DevOps and GitHub integrations
    ...
    ///
    /// Unlike the other report flags, this one is opt-in: no SARIF log is written unless it is
    /// given a path. Uploading it with `github/codeql-action/upload-sarif` puts survivors in the
    /// security tab, where they can be tracked and dismissed per mutator rather than re-reported
    /// every night.
    #[arg(long = "sarif-report", value_name = "PATH", help_heading = "Reporting")]
    ...
    /// Write the Markdown diagnosis somewhere other than the default.
    ///
    /// Every run that got as far as testing writes one; this only moves it. The analysis is a
    /// document rather than a verdict: it names what would make the run cheaper and, for every
    /// remedy, what that remedy costs in signal. It goes to a file rather than the console because
    /// it is written to be read later, shared, and pasted into a review.
    #[arg(long, value_name = "PATH", help_heading = "Reporting")]
    pub advice: Option<Utf8PathBuf>,

    /// Write the diagnostics bundle somewhere other than the default.
    ///
    /// Every run writes one; this only moves it. It is the file to attach to an issue about a slow
    /// run: one versioned document holding what the run cost, with no source text in it and the
    /// identifiers hashed, so what is being sent can be read before it is sent.
    #[arg(long, value_name = "PATH", help_heading = "Reporting")]
    pub diag_bundle: Option<Utf8PathBuf>,
...
mod tests {
    ...
    /// Report destinations are all named `--<format>-report`.
    ///
    /// `--sarif` is not accepted as an alias: nothing has been released, so there is no script to
    /// break and no reason to carry a second spelling forever.
    #[test]
    fn the_sarif_report_flag_is_named_like_its_siblings() {
        let cli = Cli::try_parse_from(["cargo gamma", "run", "--sarif-report", "a.sarif"]).expect("the canonical name parses");

        match cli.command {
            Command::Run(args) => assert_eq!(args.sarif.unwrap(), "a.sarif"),
            _ => panic!("expected run"),
        }

        _ = Cli::try_parse_from(["cargo gamma", "run", "--sarif", "a.sarif"]).expect_err("the pre-rename name is gone, not aliased");
    }

    /// Every flag taking a filesystem path presents the same `<PATH>` placeholder.
    ///
    /// `--config` once said `<FILE>` and `--scratch-dir` said `<DIR>`, which reads as though they
    ...
    fn path_valued_flags_all_use_the_same_placeholder() {
        ...
        for subcommand in command.get_subcommands() {
            for argument in subcommand.get_arguments() {
                ...
                if !matches!(id, "config" | "scratch_dir" | "dir") {
                    continue;
                }
