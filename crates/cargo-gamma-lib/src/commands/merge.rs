...
fn report_merge<H: Host>(host: &mut H, args: &MergeArgs, merged: &crate::merge::Merged, styler: Styler) -> crate::Result<()> {
    ...
    // A mutant is detected by a timeout or a memory ceiling as well as by a failing test, and the
    // remainder of the denominator holds `NoCoverage` alongside `Survived` — a mutant no test ever
    // reached, which is not a mutant a test ran past. Naming the remainder "survived" tells a
    // reader to go looking for a weak assertion where the truth is that nothing was asserted at all.
    writeln!(
        stream,
