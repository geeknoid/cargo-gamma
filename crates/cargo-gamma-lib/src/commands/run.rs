...
use crate::discover::{Killers, Plan};
...
pub(super) fn memory_policy(args: &RunArgs) -> exec::MemoryPolicy {
    ...
    let implied = if enforce {
        Some(exec::MemoryControl::Enforce)
    } else if measure {
        Some(exec::MemoryControl::Measure)
    } else {
        None
    };
...
///
/// Every run writes all four — HTML report, JSON report, Markdown advice and diagnostics bundle.
...
///
/// The gamma base is used rather than the workspace root because it is the one directory the run
/// has already established it can write to — a read-only checkout with `--scratch-dir` set is a
/// supported way to run this tool, and a default that landed in the checkout would fail there.
struct Documents {
...
impl Documents {
    /// The default names, with anything the caller named explicitly taking precedence.
    fn resolve(args: &RunArgs, root: &Utf8Path) -> Self {
        let base = exec::gamma_base(root, args.measure.scratch_dir.as_deref());

        Self {
            html: args.html_report.clone().unwrap_or_else(|| base.join("report.html")),
            json: args.json_report.clone().unwrap_or_else(|| base.join("report.json")),
            advice: args.advice.clone().unwrap_or_else(|| base.join("advice.md")),
            diag: args.diag_bundle.clone().unwrap_or_else(|| base.join("diagnostics.json")),
        }
    }
}

/// Refuses report options that name one physical output under more than one format.
///
/// All output names are settled before the first report is published. Otherwise a later format can
/// overwrite an earlier one and leave the command claiming that both artifacts were written.
fn validate_output_paths(args: &RunArgs, root: &Utf8Path) -> crate::Result<()> {
    let documents = Documents::resolve(args, root);
    let mut outputs = vec![
        ("HTML report", documents.html.as_path()),
        ("JSON report", documents.json.as_path()),
        ("advice", documents.advice.as_path()),
        ("diagnostics bundle", documents.diag.as_path()),
    ];

    if let Some(sarif) = args.sarif.as_deref() {
        outputs.push(("SARIF report", sarif));
    }
...
///
/// The verdict travels with the id rather than being thrown away, because a mutant that is not
/// retried still belongs to the population being scored. Carrying only the ids is what made a
/// second-generation run report on its own retried subset — a different, smaller population than
/// the one the threshold was written for, and a report the third generation could not be fed.
/// Says how many mutants the run record spared this run, and how many of them were not trusted.
fn report_iteration<H: Host>(
    host: &mut H,
    args: &RunArgs,
    plan: &Plan,
    adopted: usize,
    ...
    let mode = args.incremental.unwrap_or(exec::IncrementalMode::Full);
    ...
    if declined > 0 {
        writeln!(
            host.error(),
            "{} {} in the run record recorded a kill by a test that no longer exists or whose file changed; \
             they are being run again rather than carried",
            styler.verb("Rechecking"),
            quantity(declined, "mutant")
        )?;
    }

    let from_record = plan.settled_out.saturating_sub(adopted);
    ...
    writeln!(
        host.error(),
        "{} {} already settled by the run record, carried forward rather than retested",
        styler.verb("Iterating"),
        quantity(from_record, "mutant")
    )?;
...
impl IncrementalPreparation {
    fn for_run(args: &RunArgs, survey: &crate::discover::Survey, context: crate::discover::ContextDigest) -> Self {
        let base = exec::gamma_base(&survey.root, args.measure.scratch_dir.as_deref());
        let killers = survey.killers();
...
pub(super) fn run_config(args: &RunArgs, styler: Styler) -> exec::Config {
    exec::Config {
        ...
        whole_test_binaries: args.measure.whole_test_binaries,
        nextest: args.measure.nextest,
        incremental: args.incremental.unwrap_or(exec::IncrementalMode::Full),
        timeout_floor: args
            .measure
...
///
/// Incremental mode governs what is reused: `no` retests everything from scratch, `build` reuses
/// unviability results from `last-run.json`, and `full` additionally carries forward test verdicts
/// when its complete pre-run workspace snapshot and test declarations match.
///
fn adopt(
    args: &RunArgs,
    ...
    let trust = if mode.is_full() {
        crate::discover::Trust::Settled
    } else {
    ...
    let (recorded, declined, moved) = match context {
        Some(context) if !args.dry_run => {
            ...
            let empty = Killers::default();
            let trusted_killers = if mode.is_full() { killers } else { &empty };
            let (settled, declined) = record.settled_against(&survey.root, trust, trusted_killers, context, inputs);
...
fn measured<H: Host>(host: &mut H, args: &RunArgs, progress_when: When, styler: Styler) -> crate::Result<Executed> {
    ...
    progress.status(host, "Preparing", "the workspace");
    ...
    validate_output_paths(args, &survey.root)?;

    let incremental = context.map(|context| IncrementalPreparation::for_run(args, &survey, context));
    let (cached, declined, moved) = incremental.as_ref().map_or_else(
    ...
    if args.dry_run {
        ...
        report_iteration(host, args, &plan, 0, declined, styler)?;
    ...
    let mode = args.incremental.unwrap_or(exec::IncrementalMode::Full);
    if let Some(IncrementalPreparation {
        base,
        context,
        killers,
        inputs,
    }) = incremental
        && let Some(record) = crate::discover::RunRecord::from_plan_snapshot(&plan, &context, inputs, &killers)
    {
    ...
    report_iteration(host, args, &plan, adopted, declined, styler)?;
    report_idle(host, &plan, styler)?;
    ...
    if args.leak_dirs {
        let tree = exec::scratch_tree(&plan.root, args.measure.scratch_dir.as_deref());

        writeln!(host.error(), "{} {tree}", styler.verb("Kept"))?;
    }

    crate::report::summarize(host, &plan, styler, listings(args, announced))?;
    crate::report::session_notes(host, &built.session, missing_hints(mode, &plan.root), styler)?;
...
fn summary_panel(args: &RunArgs, plan: &Plan, session: &exec::Session, stuck: &[String], dropped: &[String], wall: Duration) -> String {
    // The job summary wants a fragment under the heading it already owns; `--advice` wants a whole
...
mod tests {
    ...
    use crate::testing::{BrokenHost, Sink, fails_at_every_line, workdir};
    ...
    fn disabled_incremental_mode_suppresses_missing_hints_advice() {
        ...
        assert!(missing_hints(exec::IncrementalMode::Full, root));
    ...
    fn a_closed_stream_stops_the_report_writer_at_whichever_line_it_reached() {
        ...
        fails_at_every_line(3, |host| {
            let args = RunArgs {
                ...
                html_report: Some(root.join("result.html")),
                html_external: true,
    ...
    fn a_closed_results_stream_ends_annotation_writing_successfully() {
        ...
        emit_ci(&mut BrokenHost, &args, &plan, None, Styler::new(false)).expect("closed pipe");
    }
    ...
    fn reports_write_json_html_sarif_annotations_and_summary() {
        ...
        let args = RunArgs {
            ...
            html_report: Some(root.join("reports/result.html")),
            html_external: true,
        ...
        assert!(args.html_report.as_ref().expect("html path").exists());
    ...
    /// A run that names no report path still gets both reports, under the gamma directory.
    ///
    ...
    /// forgotten.
    #[test]
    fn a_run_that_asks_for_nothing_still_writes_both_reports_under_the_gamma_directory() {
        let mut host = Sink::default();
        let plan = plan();
        let args = RunArgs::default();
        let base = exec::gamma_base(&plan.root, None);

        emit_reports(&mut host, &args, &plan, None, None, &[], Styler::new(false)).expect("reports");

        assert!(base.join("report.json").exists());
        assert!(base.join("report.html").exists());
    }

    /// Naming a path moves the report rather than switching it on, and the default location is
    /// then left alone — a caller who redirects a report does not want a second copy of it.
    #[test]
    fn naming_a_report_path_moves_it_off_the_default_instead_of_adding_to_it() {
        ...
        let base = exec::gamma_base(&plan.root, None);
        let args = RunArgs {
            json_report: Some(root.join("elsewhere/result.json")),
            ..Default::default()
        };

        emit_reports(&mut host, &args, &plan, None, None, &[], Styler::new(false)).expect("reports");

        assert!(root.join("elsewhere/result.json").exists());
        assert!(!base.join("report.json").exists());
        assert!(
            base.join("report.html").exists(),
            "the report that was not redirected keeps its default"
        );
    }

    /// The defaults follow `--scratch-dir`, because a read-only checkout run with one set has no
    /// writable `target` to put them in and would fail on the very first report.
    #[test]
    fn a_scratch_directory_moves_the_default_reports_with_it() {
        ...
        assert!(exec::gamma_base(&plan.root, Some(&scratch)).join("report.json").exists());
        assert!(!exec::gamma_base(&plan.root, None).join("report.json").exists());
    }
    ...
    fn a_default_report_path_that_cannot_be_written_fails_the_run() {
        for blocked in ["report.json", "report.html", "advice.md"] {
            let dir = workdir("run-report-blocked-");
            let scratch = Utf8PathBuf::from_path_buf(dir.path().to_path_buf()).expect("utf8");
            let plan = plan();
            let base = exec::gamma_base(&plan.root, Some(&scratch));
            ...
            let error = if blocked == "advice.md" {
    ...
    /// The advice document goes where `--advice` names it, and the diagnostic dump only appears
    /// when `--diag` asks for it.
    #[test]
    ...
    fn the_diagnostics_bundle_carries_neither_source_text_nor_a_path() {
        ...
        args.measure.jobs = Some(0);
        args.diag_bundle = Some(root.join("bundle.json"));
        ...
        let text = fs::read_to_string(root.join("bundle.json")).expect("bundle file");
    ...
    fn a_run_that_asks_for_no_advice_path_still_writes_it_under_the_gamma_directory() {
        ...
        assert!(base.join("advice.md").exists());
    }
    ...
    /// --target=…` would be surveyed as a debug host build and every item behind a gate those
    ...
    fn the_build_discovery_is_given_carries_the_command_lines_profile_and_arguments() {
        ...
        assert_eq!(build.debug_assertions, Some(false));
    }
    ...
    /// An incremental run says how much of the population the run record spared it.
    #[test]
    fn an_incremental_run_says_how_many_mutants_were_already_settled() {
        let args = RunArgs {
            incremental: Some(exec::IncrementalMode::Full),
            ..Default::default()
        };
        let mut plan = plan();
        ...
        report_iteration(&mut host, &args, &plan, 0, 0, Styler::new(false)).expect("iteration");

        assert!(host.err().contains("Iterating"), "{}", host.err());
        assert!(host.err().contains("12 mutants"), "{}", host.err());
    }

    /// An incremental run reports declined kills when tests have changed or been removed.
    #[test]
    fn an_incremental_run_reports_declined_kills() {
        let args = RunArgs {
            incremental: Some(exec::IncrementalMode::Full),
            ..Default::default()
        };
        ...
        report_iteration(&mut host, &args, &plan, 0, 3, Styler::new(false)).expect("iteration");

        assert!(host.err().contains("Rechecking"), "{}", host.err());
        assert!(host.err().contains("3 mutants"), "{}", host.err());
    }

    /// Nothing is said when incremental mode spared nothing.
    #[test]
    fn a_run_that_settled_nothing_says_nothing_about_iterating() {
        let plan = plan();
        let mut host = Sink::default();

        report_iteration(&mut host, &RunArgs::default(), &plan, 0, 0, Styler::new(false)).expect("no iterate");
        ...
        report_iteration(&mut host, &args, &plan, 0, 0, Styler::new(false)).expect("nothing settled");
    ...
    fn a_closed_stream_is_reported_by_the_iteration_note() {
        ...
        fails_at_every_line(1, |host| report_iteration(host, &args, &plan, 0, 0, Styler::new(false)));
    }
    ...
    /// An `--advice` path that cannot be written is reported as an error rather than silently
    ...
    #[test]
    fn a_broken_advice_path_is_reported_as_an_error() {
        ...
        let args = RunArgs {
            advice: Some(root.join("blocked/advice.md")),
            select: crate::commands::SelectArgs {
                dir: root,
                mutators: Some("relational.lt_to_le".to_owned()),
                ..crate::commands::SelectArgs::default()
        ...
        let error = run_session(&mut host, &args, When::Never, Styler::new(false)).expect_err("broken advice path");

        assert!(error.to_string().contains("could not write the advice"), "{error}");
    }
    ...
    #[test]
    fn run_outputs_with_identical_names_are_refused_before_reporting() {
        let directory = workdir("run-output-collision-");
        let root = Utf8PathBuf::from_path_buf(directory.path().to_path_buf()).expect("UTF-8 path");
        let output = root.join("report");
        let args = RunArgs {
            html_report: Some(output.clone()),
            json_report: Some(output),
            ..RunArgs::default()
        };

        let failure = validate_output_paths(&args, &root).expect_err("one path cannot hold two report formats");
    ...
    #[cfg(unix)]
    #[test]
    fn run_outputs_with_symlink_aliased_names_are_refused_before_reporting() {
        let directory = workdir("run-output-symlink-collision-");
        let root = Utf8PathBuf::from_path_buf(directory.path().to_path_buf()).expect("UTF-8 path");
        ...
        let args = RunArgs {
            html_report: Some(target),
            json_report: Some(alias),
            ..RunArgs::default()
        };

        assert!(validate_output_paths(&args, &root).is_err());
    }
}
