...
fn apply_all<H: Host>(host: &mut H, args: &SuppressArgs, plan: &Plan, edits: &[Edit], date: &str) -> crate::Result<Written> {
    ...
    match edit_files(host, args, plan, edits, date, &mut written) {
        ...
        Err(cause) => Err(reverted(written, cause)),
    }
}
...
fn edit_files<H: Host>(
    ...
    for file in &plan.files {
        ...
        if args.dry_run_suppress {
            ...
            match crate::elements::write_if_unchanged(&path, Some(&before), &after)? {
...
///
/// Shared with `unsuppress`, whose edit loop has the same shape and the same hazard.
pub(super) fn reverted(written: Written, cause: crate::error::Error) -> crate::error::Error {
    if written.is_empty() {
        return cause;
    }
    ...
    for entry in written.into_iter().rev() {
        match crate::elements::write_if_unchanged(&entry.path, Some(&entry.after), &entry.before) {
...
fn verify_or_revert<H: Host>(
    ...
    let result = match verified {
        Ok(result) => result,
        Err(cause) => return Err(reverted(written, cause)),
    };
    ...
    Err(reverted(written, unclean(&result)))
}
...
mod tests {
    ...
    fn a_rollback_that_fails_is_reported_with_what_went_wrong_first() {
        ...
        let error = reverted(written, error!("could not read `src/zzz_gone.rs`"));
    ...
    fn a_failure_before_the_first_write_makes_no_claim_about_reverting() {
        let error = reverted(Written::new(), error!("could not read `src/lib.rs`"));
    ...
    fn a_file_written_twice_is_restored_to_the_bytes_that_were_there_first() {
        ...
        let error = reverted(written, error!("could not read `src/other.rs`"));
