...
pub(super) fn hints_with_cargo<H: Host>(host: &mut H, args: &HintsArgs, styler: Styler, cargo: &CargoOptions) -> crate::Result<i32> {
    ...
    let base = gamma_base(&plan.root, args.scratch_dir.as_deref());
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent.as_std_path())
            .map_err(|cause| error!("could not create `{parent}` to hold the hints").caused_by(cause))?;
    }
    ...
    // Said every time rather than only on the first promotion, because the property is the reason
    // the file is allowed in version control at all and a reviewer meeting it in a diff has no
    // other way to know it. Nothing in here settles a verdict or removes a mutant from a score.
    writeln!(
        host.error(),
        "{} these hints only reorder work and pre-aim tests; they never settle a verdict or exclude a mutant",
        styler.note("Note")
    )?;
