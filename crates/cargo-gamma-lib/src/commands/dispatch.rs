...
///
/// Said after the command rather than as it happens: the places that raise a note are a worker
/// thread reaching a verdict and a command line being assembled several layers down, and neither is
/// a moment when writing to the terminal is safe — the progress display owns it. Said on the way
/// out of a failed command too, because a note about the build is often the context that makes the
/// failure legible.
...
#[cfg(target_os = "linux")]
fn relaunch_for_memory_control<H: Host>(host: &mut H, args: &super::cli::RunArgs, styler: Styler) -> Option<i32> {
    ...
    // Said before the relaunch rather than after, because from here the terminal belongs to the
    // child, and an unexplained second process is exactly the kind of thing that makes someone
    // doubt what else the tool is doing behind their back.
    let _ = writeln!(
        host.error(),
        "{} re-running inside a systemd scope, because this host does not otherwise delegate a cgroup for memory control. Pass `--no-relaunch` to run without it",
        styler.note("Note")
    );
...
pub(super) fn dispatch<H: Host>(host: &mut H, cli: Cli, styler: Styler) -> crate::Result<i32> {
    match cli.command {
        Command::Run(mut args) => {
            ...
            #[cfg(target_os = "linux")]
            if let Some(code) = relaunch_for_memory_control(host, &args, styler) {
                return Ok(code);
            }
        ...
        Command::Suppress(mut args) => {
            ...
            #[cfg(target_os = "linux")]
            if let Some(code) = relaunch_for_memory_control(host, &args.run, styler) {
                return Ok(code);
            }
...
mod tests {
    ...
    fn crate_dir(name: &str, config: Option<&str>) -> tempfile::TempDir {
        ...
        if let Some(text) = config {
            fs::create_dir(root.join(".cargo")).expect("cargo dir");
            fs::write(root.join(".cargo/gamma.toml"), text).expect("config");
        }
    ...
    /// A `.cargo/gamma.toml` selection key reaches a discovery command.
    ///
    ...
    /// Exactly the discovery commands fold `.cargo/gamma.toml` selection keys in before discovery;
