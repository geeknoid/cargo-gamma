...
mod tests {
    ...
    fn explanation_names_aliases_and_suppressions() {
        ...
        let code = explain(&mut host, &ExplainArgs { subject: "fn_value.default".to_owned() }).expect("explain");
    ...
    /// Piping into a program that exits early closes the stream, and every line has to surface it.
    #[test]
    fn a_closed_output_stream_is_reported_from_any_line() {
        // Two mutators, so the loop runs a second time and the aliases line is reached.
        fails_at_every_line(12, |host| {
            explain(host, &ExplainArgs { subject: "relational".to_owned() }).map(|_| ())
        });
    }
    ...
    fn a_family_selector_explains_each_of_its_mutators() {
        ...
        let code = explain(&mut host, &ExplainArgs { subject: "relational".to_owned() }).expect("explain");

        assert_eq!(code, EXIT_OK);
        assert!(host.out().lines().filter(|line| line.starts_with("relational.")).count() > 1, "{}", host.out());
    }
}
