...
pub struct Config {
    ...
    /// How much an incremental run reuses from the last run: `no`, `build`, or `full`.
...
pub struct Reporters {
    ...
    /// Where to write the Markdown diagnosis.
...
impl Config {
    ...
    pub fn apply(&self, args: &mut RunArgs) -> Result<()> {
        ...
        args.html_report = args.html_report.take().or_else(|| self.reporters.html.clone());
        ...
        args.advice = args.advice.take().or_else(|| self.reporters.advice.clone());
...
mod tests {
    ...
    fn reporter_paths_come_from_the_file_when_the_command_line_is_silent() {
        let config = Config::parse("[reporters]\nhtml = \"out/report.html\"\njson = \"out/report.json\"\nsarif = \"out/report.sarif\"\n")
            .expect("parses");
        let mut args = RunArgs::default();

        config.apply(&mut args).expect("the merged settings do not contradict one another");

        assert_eq!(args.html_report.as_deref(), Some(Utf8Path::new("out/report.html")));
