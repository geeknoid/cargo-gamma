...
///
/// Any mutant whose span does not lie inside `text` is skipped rather than spliced at a wrong
/// offset, because a bad offset would silently corrupt unrelated code and the resulting failures
/// would be blamed on the test suite.
///
/// # Errors
///
...
pub fn instrument_with_guards(text: &str, mutants: &[&Mutant]) -> Result<(String, HashMap<u32, Guard>)> {
    let mut sites: Vec<&Mutant> = mutants
        .iter()
        .copied()
        .filter(|mutant| mutant.span.end <= text.len() && mutant.span.start < mutant.span.end)
        .collect();
    ...
    for root in &roots {
        out.push_str(text.get(cursor..root.span.start).unwrap_or_default());
...
fn build_tree<'a>(sites: &[&'a Mutant]) -> Result<Vec<Node<'a>>> {
    ...
    for mutant in sites {
        ...
        if let Some(top) = stack.last_mut() {
            ...
            if top.span == mutant.span && top.shape == mutant.shape {
                top.mutants.push((mutant.ordinal, mutant.replacement.as_str()));
                continue;
            }
        ...
        stack.push(Node {
            ...
            shape: mutant.shape,
            mutants: vec![(mutant.ordinal, mutant.replacement.as_str())],
            children: Vec::new(),
        });
    }
...
fn render(text: &str, node: &Node<'_>, out: &mut String, spans: &mut HashMap<u32, (Range<usize>, Range<usize>)>) {
    ...
    for child in &node.children {
        out.push_str(text.get(cursor..child.span.start).unwrap_or_default());
        render(text, child, out, spans);
        cursor = child.span.end;
    }

    out.push_str(text.get(cursor..node.span.end).unwrap_or_default());
...
mod tests {
    ...
    fn a_guard_reports_the_whole_range_it_spans() {
        ...
        assert!(span.site.end.line > span.site.start.line, "a multi-line site should span multiple lines");
    }
    ...
    fn a_nested_guard_lies_outside_its_enclosing_guards_mutated_branch() {
        ...
        assert!(outer.site.start <= inner.site.start && outer.site.end >= inner.site.end, "the outer site should enclose");
        assert!(
    ...
    fn a_guarded_site_that_grows_shifts_the_guards_below_it() {
        ...
        let found = out.lines().position(|line| line.contains("a(2u32)")).and_then(|at| u32::try_from(at + 1).ok());

        assert_eq!(guards.get(&2).map(|guard| guard.site.start.line), found);
        assert_ne!(guards.get(&2).map(|guard| guard.site.start.line), Some(5), "the guard should not be on its source line");
    }
    ...
    fn mutant(span: Range<usize>, ordinal: u32, replacement: &str, shape: Shape) -> Mutant {
        Mutant {
            id: format!("m{ordinal}"),
            ordinal,
            file: Utf8PathBuf::from("test.rs"),
            package: "p".to_owned(),
            span,
            line: 1,
            column: 1,
            mutator: "test.op".to_owned(),
    ...
    fn two_arm_mutants_on_one_pattern_chain_with_and() {
        ...
        let out = apply(
            text,
            &[mutant(span.clone(), 4, "", Shape::Arm), mutant(span, 5, "", Shape::Arm)],
        );
        ...
        assert!(
            out.contains("Some(n) if !::gamma_rt::a(4u32) && !::gamma_rt::a(5u32) =>"),
            "{out}"
        );
        _ = parse_file(&out).expect("the instrumented source does not parse");
    }
    ...
    fn an_expression_site_becomes_a_parenthesized_guard() {
        ...
        assert_eq!(out, "fn f(a: i32, b: i32) -> bool { (if ::gamma_rt::a(7u32) { (a) <= (b) } else { a < b }) }");
    }
