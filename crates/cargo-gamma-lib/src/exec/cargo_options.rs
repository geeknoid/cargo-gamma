...
use crate::migrate::{adjust_lock_flags, lock_flag_reason};
...
impl CargoOptions {
    ...
    pub fn extend_build_args(&self, args: &mut Vec<String>) {
        ...
        let (extra, substituted) = adjust_lock_flags(&self.extra);

        args.extend(extra);

        if !substituted.is_empty() {
            announce_once(&lock_flag_reason(&substituted));
        }
    }
...
/// Says one thing, at most once per run, on the diagnostic stream.
///
/// A run assembles a cargo command line several times over — the check build, the test build, each
/// rollback round — so saying it where the substitution happens would repeat one decision the user
/// made once. Raised as a note rather than written out here, because nothing this far down holds a
/// `Host`; the running command says it through its own.
fn announce_once(message: &str) {
    crate::notes::note_once(format!("warning: {message}"));
}
...
mod tests {
    ...
    ///
    /// Not merely a delegation: if this ever read the environment instead of the profile and the
    /// passthrough arguments, a run built with `--profile release --target …` would be surveyed as
    /// a debug host build, and every item behind a gate those settings decide would be misjudged.
    #[test]
    ...
    /// A flag the run could not keep is not silently dropped: the substitution is announced.
    ///
    /// It goes out as a note rather than straight to `stderr` because this is several layers below
    /// anything holding a `Host`, and a message that bypasses the `Host` bypasses the colour, the
    /// width and the progress display the rest of the tool's output respects.
    #[test]
    fn substituting_a_lock_flag_is_announced_once_however_many_times_the_line_is_assembled() {
        crate::notes::alone(|| {
            let options = CargoOptions {
            ...
            assert!(raised[0].contains("`--locked` adjusted to `--offline`"), "{raised:?}");
        });
    }
