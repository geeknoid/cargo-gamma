...
mod tests {
    ...
    fn without_a_budget_nothing_is_ever_declared_stalled() {
        // An hour of silence, which no real test could afford to wait for and which nothing about
        // the answer depends on: without a budget there is no comparison to make.
        assert!(!Stall::NONE.exceeded(&silent_for(Duration::from_secs(3600))));
        assert_eq!(Stall::NONE.slack(&silent_for(Duration::from_secs(3600))), Duration::MAX);
    }
    ...
    fn a_line_of_output_clears_the_silence() {
        let progress = silent_for(Duration::from_secs(3600));
        let stall = Stall {
