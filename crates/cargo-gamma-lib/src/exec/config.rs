...
pub struct Config {
    ...
    /// The floor only ever binds for suites finishing in under ~17 seconds, where the multiplier
    ...
    /// Where to put the scratch tree and its build artifacts. `None` uses an isolated cache base
