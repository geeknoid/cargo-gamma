...
impl Converger {
    ...
    pub(super) fn stage(
        ...
        match self.converge(work, plan, self.scoped(Some(packages)), &["build", "--keep-going"], limits, events)? {
            Convergence::Built(_stdout) => Ok(None),
    ...
    fn compile(
        ...
        match self.converge(work, plan, select, &["build", "--tests", "--keep-going"], limits, events)? {
            Convergence::Built(_stdout) => {}
            stuck @ Convergence::Stuck(_) => return Ok(stuck),
        }
        ...
        self.converge(work, plan, select, &["test", "--no-run"], limits, events)
    }
