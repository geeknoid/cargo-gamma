...
//! still does not reach `S`. The relation this measures is therefore the relation the sweep needs,
...
//! such a suite. A binary whose census run was cut short — where sites genuinely went unrecorded —
//! is thrown away whole rather than trusted in part.
//!
...
/// The record the runtime writes when a site's ordinal is past the table it can record into.
///
/// Its presence means sites went unrecorded, and an unrecorded site is indistinguishable from an
/// unreached one — so the census that carries it is discarded rather than believed in part.
const OVERFLOW: u32 = u32::MAX;

/// The record the runtime writes last, at a clean exit, to vouch that the census is whole.
///
/// The runtime appends its file and flushes it as a prefix of what it wrote, so this record intact
/// at the very end proves every record before it is present too. Its absence — a truncated write, a
/// crash before exit, a failed open, a run the runtime poisoned after a short write — is the signal
/// that sites may be missing, and a census that might be missing sites is discarded whole.
///
/// Mirrors `gamma_rt`'s own `SEAL`, and the two must agree. If they ever drift this reader simply
/// distrusts every census, which loses the optimisation without ever trusting a false one.
const SEAL: u32 = u32::MAX - 2;
...
struct Reach {
    ...
    intern_index: HashMap<Box<[u32]>, u32>,
...
impl Reach {
    ...
    fn intern_set(&mut self, mut set: Vec<u32>) -> u32 {
        ...
        let _previous = self.intern_index.insert(key.clone(), index);
...
impl Census {
    ...
    pub(super) fn reaching(&self, binary: &TestBinary, ordinal: u32) -> Option<Vec<&str>> {
        let reach = self.binaries.get(&binary.path)?;
...
///
/// Never fails the run. A binary that cannot be listed, or whose tests did not all pass, simply
/// does not appear in the result, and its mutants are judged exactly as they would have been with
/// no census at all.
pub(super) fn take(work: &Workspace, binaries: &[TestBinary], jobs: usize, stall: Stall, events: &mut impl Events) -> Census {
    ...
    for binary in binaries {
        ...
        if names.is_empty() {
            // Nothing to attribute, but the claim is still real and still worth keeping: no test in
            // this binary reaches any site, so its mutants need not run it at all.
            let _previous = census.binaries.insert(binary.path.clone(), Reach::default());
            completed += 1;
            events.phase_progress(completed, total, &unit);
    ...
    let (mapped, walked) = walk(work, listed, jobs, stall, || {
        completed += 1;
        events.phase_progress(completed, total, &unit);
    });
...
fn list(work: &Workspace, binary: &TestBinary) -> Option<Vec<Box<str>>> {
    ...
    listed(&mut command, LIST_BUDGET)
}
...
fn walk(
    ...
    walk_with(work, listed, jobs, stall, sample, completed)
}

#[expect(
    clippy::too_many_lines,
    reason = "the streaming walk shares borrowed state across its scoped workers"
)]
fn walk_with<S>(
    work: &Workspace,
    listed: Vec<(&TestBinary, Vec<Box<str>>)>,
    ...
    S: Fn(&Workspace, &TestBinary, &str, &Utf8Path, Stall) -> Option<(Vec<u32>, Duration)> + Sync,
{
    ...
    let reached: Vec<Mutex<HashMap<u32, Vec<u32>>>> = listed.iter().map(|_entry| Mutex::default()).collect();
    ...
    thread::scope(|scope| {
        for worker in 0..jobs.max(1) {
            let (next, listed, offsets, reached, times, spoiled, remaining, launches) =
                (&next, &listed, &offsets, &reached, &times, &spoiled, &remaining, &launches);
                    ...
                    if !spoiled.load(Ordering::Relaxed) {
                        ...
                        match (sampler(work, binary, name, &path, stall), u32::try_from(test_at)) {
                            (Some((sites, elapsed)), Ok(test_index)) => {
                                ...
                                for site in sites {
                                    reached.entry(site).or_default().push(test_index);
                                }
                            }
    ...
    for ((binary, names), (raw_reached, (times, spoiled))) in
        listed.into_iter().zip(reached.into_iter().zip(times.into_iter().zip(spoiled)))
    {
        ...
        #[expect(clippy::unwrap_used, reason = "the lock only poisons if a worker panicked, and none can")]
        let raw_reached = raw_reached.into_inner().unwrap();

        #[expect(clippy::unwrap_used, reason = "the lock only poisons if a worker panicked, and none can")]
        let times = times.into_iter().map(|time| time.into_inner().unwrap()).collect();
...
mod tests {
    ...
    fn a_listing_is_narrowed_by_the_filters_the_user_gave() {
        ...
        // The same binary with no filter to carry prints nothing, which is what makes the case
        // above a measurement of the arguments rather than of the helper.
        work.set_test_args(vec![crate::testing::directive(SCRIPT)]);

        assert_eq!(list(&work, &binary), Some(Vec::new()));
    }
    ...
    fn an_uncensused_binary_is_not_the_same_as_one_that_reaches_nothing() {
        ...
        let _previous = census.binaries.insert(Utf8PathBuf::from("/t/a"), Reach::default());
    ...
    fn walked_counts_launches_that_happened_not_tests_that_were_listed() {
        ...
        let mut completed = 0;
        let (mapped, walked) = walk(&work, vec![(&binary, names)], 1, Stall::NONE, || completed += 1);
    ...
    fn a_clean_run_that_leaves_no_census_spoils_rather_than_reading_empty() {
        ...
        let mut completed = 0;
        let (mapped, walked) = walk(&work, vec![(&binary, vec!["quiet".into()])], 1, Stall::NONE, || completed += 1);
