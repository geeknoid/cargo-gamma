...
///
/// A spawn reports a missing program and a missing working directory with the same `ENOENT`, and
/// the two have opposite remedies, so the directory is probed to tell them apart. Naming the
/// program matters because it is not always `cargo` from `PATH`: an inherited `CARGO` wins, and a
/// stale one left over from an earlier shell points at a binary that no longer exists.
pub(super) fn spawn_failure(program: &str, work: &Workspace, cause: io::Error) -> Error {
    if cause.kind() == io::ErrorKind::NotFound && !work.root.as_std_path().is_dir() {
