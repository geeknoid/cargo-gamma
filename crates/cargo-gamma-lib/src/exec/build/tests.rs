...
use super::invoke::compile;
use super::invoke::{
    OutputLimits, Stream, drained, finish_readers, is_progress, read_pipe, read_pipe_with_limits, rendered_diagnostic, spawn_failure,
    supervise, supervise_with_limits,
};
...
/// as `NotBuilt`, and the run would report ~100% with no sign anything went wrong. When the
...
fn a_line_that_is_not_json_does_not_stop_dep_file_discovery() {
    ...
    let stdout = format!("not json at all\n{{\"reason\":\"compiler-artifact\",\"filenames\":[\"{deps}/libmine-aaaa.rmeta\"]}}\n");
...
fn guarded_workspace(prefix: &str) -> (tempfile::TempDir, Workspace) {
    ...
    // The tool's own runtime source rather than a stand-in, so a change to the guard's signature
    // breaks this fixture rather than leaving it testing a shape nothing generates any more.
    fs::write(
        runtime.join("src/lib.rs").as_std_path(),
        include_str!("../../../../cargo-gamma-rt/src/lib.rs"),
    )
