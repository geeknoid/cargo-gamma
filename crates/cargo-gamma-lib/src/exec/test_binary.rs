...
/// mutant, and a timeout counts as a detection, so the run reports a near-perfect score composed
/// entirely of mutants nothing tested. No cutoff at all is the honest answer.
...
///
/// The oracle is capped at the tests cargo itself would run here. A binary only decides a verdict
/// if its package is one the run selected — `--package`, `--workspace`, or the package owning the
/// invocation directory — so `cargo gamma` inside a crate consults that crate's suite and no more,
/// exactly as `cargo test` does. Widening is the caller's to ask for, with `--test-package` or
/// `--test-workspace`.
///
...
/// altogether. Failing both, the oracle is the packages cargo would have acted on here, which is
/// what makes a bare `cargo gamma` cost a multiple of a bare `cargo test` rather than a multiple of
/// the whole workspace.
///
...
pub(super) fn workload(
    ...
    // package. Its cost remains per-mutant when a census is present because each site has its own
    // measured set of tests.
    let mut by_package: HashMap<&str, Vec<&TestBinary>> = HashMap::new();

    for mutant in mutants
        .iter()
        .filter(|mutant| mutant.ordinal > 0 && mutant.outcome == Outcome::Pending)
    {
        let reachable = by_package.entry(&*mutant.package).or_insert_with(|| {
            binaries
                .iter()
...
mod tests {
    ...
    fn the_floor_is_a_promise_about_one_binary() {
        ...
        assert_eq!(binaries[1].budget, Some(Duration::from_secs(120)));
    }
    ...
    fn a_binary_that_earns_more_than_the_floor_keeps_its_share() {
        ...
        assert_eq!(binaries[0].budget, Some(Duration::from_secs(120)));
    }
