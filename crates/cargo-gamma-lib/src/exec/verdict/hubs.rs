...
mod loom_models {
    ...
    #[test]
    fn a_pulse_wakeup_is_never_lost_under_any_interleaving() {
        loom::model(|| {
    ...
    /// decrement.
    #[test]
    fn a_readers_gauge_returns_to_exactly_zero_under_any_interleaving() {
        loom::model(|| {
    ...
    #[test]
    fn a_readers_peak_never_understates_four_concurrent_starts() {
        loom::model(|| {
            let readers = Arc::new(Readers::new());
