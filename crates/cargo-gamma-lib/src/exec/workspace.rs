...
///
/// The answer is always absolute. A relative `--scratch-dir` is resolved against the current
...
pub fn gamma_base(root: &Utf8Path, scratch: Option<&Utf8Path>) -> Utf8PathBuf {
    let base = scratch.map_or_else(
        ...
        |directory| directory.join("gamma"),
    );
...
fn ensure_copy_terminates(source: &Utf8Path, base: &Utf8Path) -> Result<()> {
    ...
    if prunes(&real_source, &real_base) {
        return Err(error!(
            "the scratch directory `{base}` is `{real_base}`, inside the workspace at `{source}`, but is \
             ...
             Point --scratch-dir at a directory that is really outside the workspace."
        )
        .usage());
    }
    ...
    Err(error!(
        "the scratch directory `{base}` is the workspace at `{source}` itself, so copying the \
         ...
         Point --scratch-dir at a directory outside the workspace."
    )
    .usage())
}
...
/// builds differ turns `--scratch-dir` into an undocumented build-input switch, so the unsupported
...
fn ensure_vcs_visibility(source: &Utf8Path, scratch: &Utf8Path) -> Result<()> {
    ...
    Err(error!(
        "`--scratch-dir` would relocate the scratch tree to `{scratch}`, where build scripts cannot see VCS metadata available from `{source}`: {}. \
         Use a scratch directory beneath the same repository, or remove the build-time VCS dependency.",
...
fn claim(base: &Utf8Path) -> Result<File> {
    ...
    match try_lock(&file) {
        ...
        Err(TryLockError::Error(cause)) => {
            return Err(error!(
                "the scratch lock at `{path}` could not be taken.\n\
                 This filesystem may not support locking; --scratch-dir on a local filesystem will."
            )
            .caused_by(cause));
        }
    }
...
mod tests {
    ...
    fn scratch_tree_is_derived_from_the_same_base_as_prepare() {
        ...
        assert_eq!(
            ...
            gamma_base(Utf8Path::new("/workspace"), None).join("tree")
        );
        assert_eq!(
    ...
    /// A relative `--scratch-dir` reaches the copy as the path the user typed, and the copy prunes
    ...
    #[test]
    fn a_relative_scratch_directory_is_resolved_against_the_current_directory() {
        ...
        assert_eq!(base, cwd.join("scratch/here/gamma"));
    ...
    /// An absolute scratch directory outside the workspace is the arrangement everything else is
    /// built on, and must come back exactly as given — a run whose scratch path moved between two
    /// invocations rebuilds from cold.
    #[test]
    fn an_absolute_scratch_directory_outside_the_workspace_is_left_alone() {
        let base = gamma_base(Utf8Path::new("/workspace"), Some(Utf8Path::new("/elsewhere/scratch")));

        assert_eq!(base, Utf8PathBuf::from("/elsewhere/scratch/gamma"));
    ...
    fn a_scratch_directory_the_copy_cannot_prune_is_refused() {
        let source = Utf8Path::new("/workspace/gamma");
        let base = gamma_base(source, Some(Utf8Path::new("/workspace")));
        ...
        assert!(failure.to_string().contains("/workspace/gamma"), "{failure}");
        assert!(failure.to_string().contains("--scratch-dir"), "{failure}");
    }
    ...
    fn preparing_with_a_scratch_directory_the_copy_cannot_prune_fails_before_copying() {
        ...
        assert!(
            !source.join("tree").as_std_path().exists(),
    ...
    fn a_scratch_path_that_names_nothing_is_reduced_to_one_spelling() {
        assert_eq!(absolute(Utf8Path::new("/a/./b/../c")), Utf8PathBuf::from("/a/c"));

        // Nothing precedes a leading `..`, so it is left to mean what the filesystem says.
        assert_eq!(absolute(Utf8Path::new("/../a")), Utf8PathBuf::from("/../a"));
    }
