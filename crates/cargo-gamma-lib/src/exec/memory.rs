...
impl MemoryPolicy {
    ...
    pub const fn disabled(&self) -> Self {
        Self { control: MemoryControl::Off, ..*self }
    }
...
pub(super) struct Request {
    /// Measure the peak memory of the whole subtree.
    pub(super) meter: bool,

    /// Stop the subtree if its aggregate memory passes this many bytes.
    pub(super) limit: Option<u64>,
}

impl Request {
    /// Whether this asks the platform for anything at all.
    pub(super) const fn wanted(&self) -> bool {
...
pub(super) struct Usage {
    /// The highest aggregate memory the subtree reached, when the host could measure it.
    pub(super) peak: Option<u64>,

    /// Whether the kernel itself reported stopping the workload for passing the ceiling.
    ///
    /// This is a kernel-observed cause rather than an inference from an exit status, which is what
    /// makes it worth a verdict of its own. On Linux it is an `oom` or `oom_kill` event recorded
    /// against this invocation's own cgroup; on Windows it is the job object's own accounting
    /// saying the job reached the limit configured for it.
...
pub fn support() -> Result<(), String> {
    #[cfg(target_os = "linux")]
    {
        cgroup::root().map(|_root| ()).map_err(str::to_owned)
    }
    ...
    {
        Err("bounding a test subtree's memory needs cgroup v2 on Linux or a job object on Windows, \
...
mod tests {
    ...
    fn a_small_baseline_peak_gets_the_absolute_headroom() {
        ...
        // noticed it.
        assert_eq!(policy.ceiling(Some(4 * 1024 * 1024), true), Some(4 * 1024 * 1024 + DEFAULT_HEADROOM));
    }
    ...
    fn an_uncalibrated_run_gets_no_derived_ceiling_but_keeps_an_explicit_one() {
        ...
        let explicit = MemoryPolicy { limit: Some(4096), ..policy };
    ...
    fn a_request_that_asks_for_nothing_is_recognized_as_such() {
        assert!(!Request::default().wanted());
        assert!(Request { meter: true, limit: None }.wanted());
        assert!(Request { meter: false, limit: Some(1) }.wanted());
    }
