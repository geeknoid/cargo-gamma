...
///
/// Every verdict that counts as a detection is confirmed by a second run before it is believed. A
/// false detection does not merely lose information: it inflates the mutation score by crediting
/// the suite with a kill it never made, which is the one direction this tool must not be wrong in.
///
...
/// Says on the diagnostic stream that a named failure happened alongside a memory verdict.
///
/// The name is the useful half — it is what a reader goes and opens — so it is what the verdict
/// carries, and the memory situation is reported beside it rather than replacing it.
fn announce_memory(binary: &TestBinary, test: &str, peak: Option<u64>, limit: u64) {
    let reached = peak.map_or_else(String::new, |peak| format!(", reaching {}", crate::report::bytes(peak)));

    announce(&format!(
        "test `{test}` in `{}` failed in a run that also passed the {} it was allowed{reached}. The \
         failing test is what the verdict names; the memory may be what caused it.",
        binary.path,
...
fn configure(
    ...
    let _ = command
        .stdout(Stdio::piped())
        .stderr(if under_nextest { Stdio::piped() } else { Stdio::null() })
    ...
    loop {
        ...
        match ask_after(&mut subtree) {
            Ok(Some(status)) => {
                ...
                return (prefer_named(binary, verdict, usage.peak, ceiling), usage);
            }

            Ok(None) => {
                // A test has already announced that it failed, so the verdict is settled and every
                // test after it would be paid for to learn nothing. This is the whole point of
                // watching the output rather than only the exit status.
                if let Some(name) = failure(progress) {
                    let (usage, ceiling) = cut_short(&mut subtree, request);

                    return (cut_by_named_failure(binary, name, usage.peak, ceiling), usage);
                }
...
/// ceiling is reported beside the name rather than instead of it.
///
...
fn prefer_named(binary: &TestBinary, verdict: Verdict, peak: Option<u64>, ceiling: Option<u64>) -> Verdict {
    let Some(limit) = ceiling else {
        return verdict;
    };

    match verdict {
        Verdict::Failed(Some(name)) => {
            announce_memory(binary, &name, peak, limit);
...
fn cut_by_named_failure(binary: &TestBinary, name: String, peak: Option<u64>, ceiling: Option<u64>) -> Verdict {
    prefer_named(binary, Verdict::Failed(Some(name)), peak, ceiling)
}
...
/// The first test the harness announced as failed, if the output is being watched and one has.
fn failure(progress: &Mutex<Progress>) -> Option<String> {
    #[expect(clippy::unwrap_used, reason = "the reader only panics if the whole process is unwinding")]
...
fn readers(subtree: &mut ProcessTree, progress: &Arc<Mutex<Progress>>, pulse: &Arc<Pulse>) -> Receiver<(Vec<u8>, bool)> {
    ...
    // Stderr is piped only under nextest, which reports everything there. A binary run directly has
    // it closed, and this simply finds nothing to start.
        ...
        match reader.read_until(b'\n', &mut line) {
            ...
            Ok(_read) => {
                ...
                let announced = {
                    ...
                    !before && progress.failed.is_some()
                };

                // Only the *first* announced failure wakes the waiter. Waking on every line would
                // turn a chatty binary into a spin, and a failure is the one thing in the output
                // that settles the verdict on its own.
                if announced {
                    pulse.signal();
                }
...
mod tests {
    ...
    /// A memory ceiling reached in the same run as a named test failure is reported beside the
    /// verdict rather than in place of it — and it is reported through the output seam.
    ///
    /// The name is what the verdict carries, because it is what a reader goes and opens. The
    /// memory situation is the context that says whether the failing test is the fault or the
    /// symptom, and it is raised as a note because this is reached on a worker thread while the
    /// progress display owns the terminal.
    #[test]
    fn a_memory_ceiling_reached_alongside_a_named_failure_is_announced_beside_it() {
        crate::notes::alone(|| {
            announce_memory(
                &crate::testing::helper(),
                "suite::the_failing_one",
                Some(2 * 1024 * 1024),
                1024 * 1024,
            );
            ...
            assert_eq!(raised.len(), 1, "{raised:?}");
            assert!(raised[0].contains("suite::the_failing_one"), "{raised:?}");
            assert!(raised[0].contains("1.0 MB"), "the ceiling is not named: {raised:?}");
            assert!(raised[0].contains("2.0 MB"), "the peak is not named: {raised:?}");
        });
    }

    /// A run that could not measure the peak still says what the ceiling was.
    #[test]
    fn a_memory_ceiling_reached_without_a_measured_peak_still_names_the_ceiling() {
        crate::notes::alone(|| {
            announce_memory(&crate::testing::helper(), "suite::the_failing_one", None, 1024 * 1024);

            let raised = crate::notes::drain();

            assert_eq!(raised.len(), 1, "{raised:?}");
            assert!(raised[0].contains("1.0 MB"), "{raised:?}");
            assert!(!raised[0].contains("reaching"), "a peak nobody measured was reported: {raised:?}");
        });
    }
    ...
    fn a_named_failure_outranks_the_ceiling_the_same_run_crossed() {
        let binary = crate::testing::test_binary("/w/target/debug/deps/unit-abc");
        ...
        let anonymous = prefer_named(&binary, Verdict::Failed(None), Some(300), Some(256));
        ...
        let unmetered = prefer_named(&binary, Verdict::Unmetered("no cgroup".to_owned()), None, Some(256));

        assert!(matches!(unmetered, Verdict::Unmetered(_)), "{unmetered:?}");

        // With no ceiling crossed, whatever was settled stands untouched.
        let plain = prefer_named(&binary, Verdict::Failed(Some("module::case".to_owned())), Some(1), None);
    ...
    fn a_run_cut_short_by_a_named_failure_keeps_the_name_beside_the_ceiling() {
        let binary = crate::testing::test_binary("/w/target/debug/deps/unit-abc");
        let named = cut_by_named_failure(&binary, "module::case".to_owned(), Some(300), Some(256));

        assert_eq!(named, Verdict::Failed(Some("module::case".to_owned())));

        // With no ceiling crossed there is only the failure, which is the ordinary shape of this.
        let plain = cut_by_named_failure(&binary, "module::case".to_owned(), None, None);
