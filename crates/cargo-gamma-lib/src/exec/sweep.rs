...
use super::test_binary::{TestBinary, TestScope, reaches};
...
/// The binaries that can reach each package holding pending work.
///
/// Reachability is a property of the mutant's *package*, and a package holds many mutants, so it is
/// worked out once per package. Deriving it per mutant reruns the same filter over every binary in
/// the workspace thousands of times for an answer that cannot have changed.
fn reachability<'a>(
    pending: &[usize],
    plan: &Plan,
    binaries: &'a [TestBinary],
    scope: &TestScope<'_>,
) -> crate::HashMap<String, Vec<&'a TestBinary>> {
    let mut sets: crate::HashMap<String, Vec<&TestBinary>> = crate::HashMap::default();

    for position in pending {
        let package = &*plan.mutants[*position].package;

        if !sets.contains_key(package) {
            let mut found: Vec<&TestBinary> = binaries.iter().filter(|binary| reaches(binary, package, plan, scope)).collect();

            order_reachable(&mut found, package);

            let _fresh = sets.insert(package.to_owned(), found);
        }
    }
...
///
/// Exact per-mutant and learned file-local killers are applied later by [`judge_ordered`] and
/// therefore still take precedence over this cold-run order.
fn order_reachable(binaries: &mut [&TestBinary], mutant_package: &str) {
...
fn mutant_cost(
    position: usize,
    plan: &Plan,
    sets: &crate::HashMap<String, Vec<&TestBinary>>,
    census: &Census,
    killers: &Killers,
) -> Duration {
    ...
    for binary in binaries {
        match census.work(binary, mutant.ordinal) {
            super::census::CensusWork::Selected(duration) => {
                total += duration;
                used_census = true;
            }
            super::census::CensusWork::Whole => total += binary.baseline,
            super::census::CensusWork::Uncovered => {}
        }
    }
...
fn schedule(pending: &mut [usize], plan: &Plan, sets: &crate::HashMap<String, Vec<&TestBinary>>, census: &Census, killers: &Killers) {
    let cost = |position: &usize| -> Duration { mutant_cost(*position, plan, sets, census, killers) };

    pending.sort_by(|left, right| cost(right).cmp(&cost(left)).then(left.cmp(right)));
...
pub(super) fn test_all(
    ...
    let sets = reachability(&pending, plan, binaries, &scope);

    schedule(&mut pending, plan, &sets, sweep.census, killers);
    ...
    let reachable: Vec<&Vec<&TestBinary>> = pending.iter().map(|position| &sets[&*plan.mutants[*position].package]).collect();
...
/// sweep runs unchanged, which is what makes this unable to move a verdict: a probe that does not
/// convict leaves the run doing exactly what it would have done without the map. A timeout or a
/// memory verdict from one filtered test in particular says nothing, because the budget and the
/// ceiling it was measured against are the whole binary's.
///
/// The one thing that does change is which detection a mutant is credited to when more than one
/// binary would have reached one. A mutant whose recorded killer is the third binary, and whose
/// first binary would have timed out, is now reported killed rather than timed out. Both count as
/// caught, so no score moves; the more specific finding is the better one to report.
fn probe(
    work: &Workspace,
    ordinal: u32,
    binary: &TestBinary,
...
///
/// When an earlier run recorded which test caught this mutant, that one test is tried first. It is
/// a guess and it is checked, never believed: see [`probe`].
#[cfg(test)]
fn judge(
    work: &Workspace,
    ordinal: u32,
...
fn judge_ordered(
    ...
    if let Some(hint) = hint
        && let Some(binary) = reachable.iter().find(|binary| hint.names(&binary.package, &binary.target))
    ...
    let preferred = file_hint.and_then(|hint| reachable.iter().copied().find(|binary| hint.names(&binary.package, &binary.target)));
    ...
    for binary in ordered {
        // `None` is "run the whole binary": either nothing is known, or most of its tests reach
        // the site and naming them would cost more than it saves. Only a measured empty list means
        // no test reaches the site and permits skipping the binary.
        let selected = sweep.census.reaching(binary, ordinal);
        let only = match selected.as_deref() {
            None => Only::All,
            Some([]) => continue,
            Some(names) => Only::These(names),
        };
...
mod tests {
    ...
    fn cold_runs_put_own_package_binaries_before_cheaper_dependents() {
        ...
        let sets = reachability(&pending, &plan, &binaries, &NARROW);
        let ordered = &sets["subject"];
    ...
    fn a_site_most_of_the_suite_reaches_is_run_whole_rather_than_called_uncovered() {
        let (_directory, work, mut plan, binaries) = harness("exit 0", Duration::from_secs(60));
        let scope = TestScope {
            packages: &[],
            whole_workspace: true,
        };
    ...
    fn helper_harness(script: &[&str], budget: Duration) -> (tempfile::TempDir, Workspace, Plan, Vec<TestBinary>) {
    ...
    fn the_test_that_caught_a_mutant_last_time_is_tried_first() {
        let (_directory, work, _plan, binaries) = helper_harness(ONLY_WHEN_FILTERED, Duration::from_secs(30));
        let reachable: Vec<&TestBinary> = binaries.iter().collect();
    ...
    fn a_hint_naming_a_test_the_users_filter_excludes_does_not_convict() {
        let (_directory, mut work, _plan, binaries) = helper_harness(ONLY_WHEN_FILTERED, Duration::from_secs(30));
        let reachable: Vec<&TestBinary> = binaries.iter().collect();
    ...
    fn a_mutant_with_a_timeout_multiplier_overrides_the_binary_budget() {
        // The script sleeps 150ms. With 50ms baseline and default 1.0 multiplier (budget 50ms), it times out.
        // With a 5.0 multiplier (budget 250ms), it passes and therefore survives.
        let (_directory, work, _plan, mut binaries) = harness("sleep 0.15; exit 0", Duration::from_millis(50));
        ...
        let with_override = judge(&work, 1, &reachable, None, Some(5.0), sweep, &Tally::default());
        assert!(
