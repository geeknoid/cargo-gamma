...
use super::test_binary::{
    TestBinary, TestScope, admits_target, build_packages, calibrate, oracle_packages, reaches, reaching_packages, restrict, unmatched_test,
    workload,
};
...
pub fn run(survey: &Survey, selection: &Selection, config: &Config, events: &mut impl Events) -> Result<Measured> {
    ...
    // it with.
    let base = gamma_base(&survey.root, config.scratch_dir.as_deref());
    ...
    let census = if config.whole_test_binaries {
        Census::default()
    } else {
        census::take(&built.work, &built.session.binaries, config.jobs, stall, events)
    };
    ...
    let work = workload(
        ...
        &plan,
        &scope,
        (!config.whole_test_binaries).then_some(&census),
    );
    ...
    let swept = test_all(&built.work, &mut plan, &built.session.binaries, sweep, scope, &mut killers, events);
...
pub fn measure(survey: &Survey, selection: &Selection, config: &Config, events: &mut impl Events) -> Result<Measured> {
    ...
    // Checked against the filters so that a caller who has already excluded one is not told again.
    warn_about_compile_fail_targets(&survey.compile_fail, config, events);
    ...
    let oracle = Oracle { packages, whole_workspace };
    let scope = oracle.scope();
...
fn warn_about_compile_fail_targets(targets: &[CompileFailTarget], config: &Config, events: &mut dyn Events) {
    let admitted: Vec<_> = targets
        .iter()
        .filter(|target| admits_target(&target.target, &config.include_tests, &config.exclude_tests))
        .cloned()
        .collect();
...
fn warn_about_an_empty_oracle(
    ...
    events.warn(&format!(
        "no test in this run's scope reaches the code being mutated, so every mutant will be reported as uncovered. The oracle is capped at what `cargo test` would run here, and `{named}`{others} also links this code. Add `--test-package {}` to let its tests decide a verdict, or `--test-workspace` for every package's",
        helpers[0]
    ));
}
...
/// loses that race is recorded as a timeout, which counts as a kill.
fn take_baseline(
    work: &Workspace,
...
mod tests {
    ...
    fn the_default_stall_budget_is_ten_times_the_measured_quiet_period() {
        ...
        let baseline = measured(Duration::from_secs(600), Duration::from_secs(3));
    ...
    fn a_suite_that_never_goes_quiet_gets_the_stall_floor_rather_than_zero() {
        let config = Config::default();
        let baseline = measured(Duration::from_secs(600), Duration::ZERO);
    ...
    fn a_compile_fail_target_is_named_before_anything_is_built() {
        ...
        warn_about_compile_fail_targets(&[target("router_compile_fail")], &config_with(&[]), &mut events);
    ...
    fn a_target_excluded_by_a_glob_is_not_warned_about() {
        ...
        warn_about_compile_fail_targets(&[target("router_compile_fail")], &config_with(&["*_compile_fail"]), &mut events);
    ...
    fn a_workspace_without_one_is_silent() {
        ...
        warn_about_compile_fail_targets(&[], &config_with(&[]), &mut events);
