...
pub fn summary(mutants: &[Mutant], root: &Utf8Path) -> String {
    ...
    // counted as killed are exactly `Summary::detected` — killed, timed out, or stopped by the
    // memory limit, which is what the score puts in its numerator — and the total is
    // `Summary::valid`, its denominator. Adding the counts up here instead is how the two came
    // apart, with a score that counted memory kills sitting next to a breakdown that did not, and a
    // reader left unable to reconcile a number with the figures printed beside it.
    //
...
///
/// A `match`, so a new outcome cannot reach a report without a name; the three detected outcomes
/// share a "killed" prefix because the headline above counts them together, and a reader adding the
/// rows up needs to see which ones it added.
const fn label(outcome: Outcome) -> &'static str {
    match outcome {
        Outcome::Killed => "Killed",
        Outcome::Timeout => "Killed by timeout",
        Outcome::OutOfMemory => "Killed by memory limit",
...
mod tests {
    ...
    /// A memory kill counts toward the score, so it has to count in the sentence that explains the
    /// score and in the table underneath it.
    ///
    /// The population is deliberately lopsided — every scoring outcome a different size — so that
    /// dropping any one of them from either place changes a number this test names. detected =
    /// 3 killed + 2 timed out + 4 out of memory = 9, valid = 9 + 1 survived + 5 uncovered = 15,
    /// score = 60.0%.
    #[test]
    fn the_headline_and_the_table_account_for_a_memory_kill() {
        ...
        // score's complement, which is every mutant nothing detected — the 1 survivor and the 5
        // uncovered mutants the table below lists apart — so it is labelled for that and not for
        // the one row of the two it would otherwise contradict.
        assert!(
            text.contains("**Score 60.0%** — 9 detected, 6 not detected of 15 mutants."),
            "{text}"
        );
        assert_eq!(totals.detected(), 9);
        ...
        assert!(text.contains("| Killed by timeout | 2 |"), "{text}");
        assert!(text.contains("| Killed by memory limit | 4 |"), "{text}");
        assert!(text.contains("| Survived | 1 |"), "{text}");
        assert!(text.contains("| Uncovered | 5 |"), "{text}");

        // Every row that scores adds up to the headline, and so to the score above it.
        let scoring = [("Killed", 3), ("Killed by timeout", 2), ("Killed by memory limit", 4)];
    ...
    /// A run with nothing but memory kills scores 100%, and says so with the same numbers.
    #[test]
    fn a_memory_kill_is_the_whole_breakdown_when_it_is_the_whole_run() {
        ...
        assert!(
            text.contains("**Score 100.0%** — 1 detected, 0 not detected of 1 mutants."),
            "{text}"
        );
        assert!(text.contains("| Killed by memory limit | 1 |"), "{text}");
    }
