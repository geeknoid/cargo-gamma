...
impl Build {
    ...
    ///
    /// The workspace forbids writing the process environment — see `docs/DESIGN.md` — so the
    /// variables cargo reads are taken as a value rather than looked up wherever they are needed.
...
mod tests {
    ...
    fn descendant_non_dot_cargo_home_reaches_resolution_and_record_settings_once() {
        ...
        let source_prefix = format!("cargo-config:{}:", root.join("cargo/config.toml"));

        assert_eq!(build.cfgs, ["from_descendant_home"]);
        assert_eq!(
            settings.iter().filter(|part| part.starts_with(&source_prefix)).count(),
            1,
