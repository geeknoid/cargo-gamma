...
/// Raises one diagnostic line unless the same line is already waiting to be said.
pub(crate) fn note_once(message: impl Into<String>) {
    ...
    if pending.lines.contains(&message) {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn a_repeated_note_is_kept_only_once() {
        alone(|| {
            note_once("the same decision");
            note_once("the same decision");
            note_once("a different one");

            assert_eq!(drain(), vec!["the same decision".to_owned(), "a different one".to_owned()]);
        });
    }

    #[test]
    fn a_note_repeated_after_a_drain_is_said_again() {
        alone(|| {
            note_once("worth saying twice, once each time");
    ...
    #[test]
    fn a_repeat_past_the_cap_is_neither_kept_nor_counted() {
        alone(|| {
            note("the first");
            ...
            assert_eq!(drained.len(), LIMIT, "{drained:?}");
        });
    }
