...
/// the schema's and not this tool's. `Timeout` is detected in the schema's own metric, which is the
/// position [`status_of`](crate::elements) takes when writing one, so the score a viewer computes
/// and the score printed here agree by construction. `RuntimeError` is excluded because the schema
/// keeps it out of the denominator: counting it against the score would make one `merge` invocation
/// print a number the document it wrote contradicts.
///
...
pub(super) fn scoring(status: &str) -> Option<Scoring> {
    match status {
        "Killed" | "Timeout" => Some(Scoring::Detected),
        "Survived" | "NoCoverage" => Some(Scoring::Undetected),
