...
mod tests {
    ...
    /// rejected suppression or value does — the timeout family's counterpart, and the only test
    /// that drives `inert_timeout` at all. A regression that returned the item unchanged from the
    /// `Err` arm (dropping the `compile_error!`) would let a malformed multiplier compile clean.
    #[test]
