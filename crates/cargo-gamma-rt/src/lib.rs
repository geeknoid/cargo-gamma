...
/// [`CENSUS`], so the
/// environment cannot name it — and it is neither [`OVERFLOW`] nor a site id: [`note`] diverts every
/// id at or above [`SITES`] to the [`OVERFLOW`] path, so no larger id ever reaches the file.
/// That bound is what actually keeps the encoding unambiguous, and widening [`SITES`] past
/// `u32::MAX - 2` would break it. It is also not [`CENSUS`], which never reaches the file. The
/// reader in `cargo-gamma-lib` mirrors this value, and the two must agree; if they ever drift the
/// reader simply distrusts every census, which loses the optimisation without ever trusting a false
/// one.
#[cfg(any(unix, windows))]
...
///
/// This is called only by the loader/CRT constructor below, before `main` can execute user code
/// or start user threads. That timing is essential: C environment access itself races with
/// environment mutation, so deferring even a copying read to a guard would make safe instrumented
/// code capable of undefined behavior. Guard calls only load [`ACTIVE`] afterwards.
#[cfg(any(unix, windows))]
fn capture_selection() -> u32 {
    ...
    if capture_census_path() { CENSUS } else { capture_active() }
}
...
fn capture_active() -> u32 {
    let value = environment(ACTIVE_VAR_C);
    ...
    // SAFETY: `environment` returned a value readable through its NUL terminator, which ends this
    // bounded copy. The constructor has exclusive startup access to the environment.
    let Some(length) = (unsafe { copy_c_string(value.cast(), &mut buffer) }) else {
        return NONE;
    };
...
fn capture_census_path() -> bool {
    let value = environment(CENSUS_VAR_C);

    if value.is_null() {
        return false;
    }

    // SAFETY: `environment` returned a valid C string during startup, so its first byte is
    // readable. The copy below observes the same startup-only precondition.
    if unsafe { *value } == 0 {
        return false;
    }
    ...
    // SAFETY: `environment` returned a value readable through its NUL terminator, which ends this
    // bounded copy; this copies rather than retaining an environment-owned pointer.
    let Some(length) = (unsafe { copy_c_string(value.cast(), path) }) else {
        return true;
    };
...
/// Reads one Unix environment value during startup.
///
/// This is the sole Unix environment seam. Keeping it here makes it mechanically apparent that
/// guard code has no route to `getenv`, and lets the unit regression count every such access.
#[cfg(unix)]
fn environment(name: &[u8]) -> *const core::ffi::c_char {
    #[cfg(test)]
    let _previous = STARTUP_ENVIRONMENT_READS.fetch_add(1, Ordering::Relaxed);

    // SAFETY: every caller supplies one of this module's NUL-terminated constant names. This
    // function is called only by the startup constructor, before user threads can mutate `environ`.
...
#[cfg(any(unix, windows))]
const _: () = assert!(OUTPUT_BUFFER.is_multiple_of(size_of::<u32>()));
...
/// `main`, when no user thread can mutate the environment. It also lets the seal cover a test that
/// touches no instrumented site: there is no first guard to hang registration off.
#[cfg(any(unix, windows))]
extern "C" fn install() {
...
///     if empty { Either::L(core::iter::empty()) } else { Either::R(0..3) }
/// }
///
...
mod tests {
    ...
    fn the_reserved_ordinal_is_inactive_in_baseline_census_and_mutant_processes() {
        ...
        let baseline = std::process::Command::new(&executable)
            .args(["--exact", "tests::the_child_reports_guard_selection", "--nocapture"])
    ...
    fn a_census_records_each_site_it_reached_exactly_once() {
        assert_eq!(census_of("tests::the_child_walks_a_few_sites"), vec![3, 9]);
    }

    #[test]
    #[cfg(any(unix, windows))]
    fn a_census_does_not_touch_its_file_until_process_exit() {
        assert_eq!(census_of("tests::the_child_buffers_its_sites"), vec![17]);
    }
    ...
    fn a_census_serializes_more_than_one_output_batch_in_order() {
        ...
        assert_eq!(census_of("tests::the_child_fills_multiple_output_batches"), expected);
    }
    ...
    fn a_census_that_reached_no_site_is_still_sealed() {
        ...
        assert_eq!(census_of("tests::the_child_touches_no_sites"), Vec::<u32>::new());
    }

    #[test]
    #[cfg(any(unix, windows))]
    fn a_site_claimed_while_sealing_waits_is_written_before_the_seal() {
        assert_eq!(census_of("tests::the_child_holds_a_lease_before_claiming_a_site"), vec![79]);
    }

    #[test]
    #[cfg(any(unix, windows))]
    fn a_short_exit_write_leaves_the_census_unsealed() {
        let bytes = census_bytes_of("tests::the_child_forces_a_short_exit_write");
    ...
    fn a_site_past_the_table_marks_the_whole_census_untrustworthy() {
        ...
        assert_eq!(census_of("tests::the_child_walks_past_the_table"), vec![1, 2, OVERFLOW]);
    }
...
mod loom_models {
    ...
    #[test]
    fn record_versus_seal_never_seals_over_an_admitted_record() {
        loom::model(|| {
