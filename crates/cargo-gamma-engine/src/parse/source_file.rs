...
impl SourceFile {
    ...
    pub fn parse(path: impl Into<Utf8PathBuf>, text: String) -> Result<Self> {
        ...
        if let Some(at) = nesting::beyond(&text, &comments, nesting::NESTING_LIMIT) {
            ...
            return Err(error!(
                "{path}:{line}: nests deeper than {} levels of brackets, prefix operators, chained operators or postfix expressions,                  which is deeper than this tool can walk without running out of stack",
