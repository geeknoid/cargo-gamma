...
pub(super) fn indexes(file: &File, selection: &Selection) -> Indexes {
    let mut walk = Walk {
        ...
        // `Default` to reach for, and `result.ok_to_err` to know whether the `Err` it would write
        // could be built at all. Missing the second cost five mutants when this gate was first
        // written, which is what a gate on an index has to be checked against.
