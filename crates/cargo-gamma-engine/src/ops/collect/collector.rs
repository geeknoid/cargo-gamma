...
use syn::{
    ...
    Generics, ImplItemConst, ImplItemFn, ItemConst, ItemFn, ItemImpl, ItemMod, ItemStatic, ItemTrait, Lit, Local, Macro, Member, Pat,
    RangeLimits, ReturnType, Signature, Stmt, TraitItemConst, TraitItemFn, Type, UnOp, Variant,
};
...
use indexes::{NumericUses, indexes};
use noop::is_noop;
use predicates::{
...
impl<'a> Collector<'a> {
    ...
    pub(super) fn new(
        ...
        let indexes = indexes(&file.ast, selection);
    ...
    fn emit_at(
        ...
        let Ok(at) = u32::try_from(self.candidates.len()) else {
            return;
        };
    ...
    fn struct_fields(&mut self, node: &ExprStruct) {
        ...
        for (index, field) in node.fields.iter().enumerate() {
            ...
            let (Some(head), Some(tail)) = (self.file.text.get(whole.start..from), self.file.text.get(to..whole.end)) else {
                continue;
            };
...
impl<'ast> Visit<'ast> for Collector<'_> {
    ...
    fn visit_item_trait(&mut self, node: &'ast ItemTrait) {
        ...
        self.with_defaulted_parameters(&node.generics, |collector| visit::visit_item_trait(collector, node));
    }
