...
/// A file with no attribute produces exactly what it produced before.
///
/// The one guarantee that makes the feature safe to land: an id is a stable name for a mutant
/// across runs, and a change that shifted ids for unannotated code would invalidate every stored
/// verdict and every suppression in every project that never uses the attribute.
#[test]
fn a_file_that_states_nothing_is_collected_exactly_as_before() {
