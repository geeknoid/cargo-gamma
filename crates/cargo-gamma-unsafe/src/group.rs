...
/// # Errors
///
/// Rejects non-positive identifiers without making a system call. Other errors are the operating
/// system's reason the group could not be signalled.
pub fn kill(group: i32) -> io::Result<()> {
    if group <= 0 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "process-group identifiers must be positive",
        ));
    }
...
mod tests {
    ...
    #[test]
    fn killing_a_non_positive_group_is_rejected_without_signalling() {
        for group in [0, -1, i32::MIN] {
            let error = kill(group).expect_err("a non-positive group must be refused");
