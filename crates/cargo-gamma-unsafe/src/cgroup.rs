...
//! it delegates to. Where none of that is possible, `support` says so with the reason, and
//! the run
...
fn vacate(own: &Path, supervisor: &Path) -> Result<(), String> {
    ...
    // #[gamma::skip(literal.int_increment, range.exclusive_to_inclusive, expr.decrement, expr.increment, reason = "the exact retry boundary is observable only if the kernel process list changes on precisely that pass; fixed and empty lists already cover the two deterministic outcomes")]
    for _attempt in 0..MIGRATION_ATTEMPTS {
        let listed =
            fs::read_to_string(own.join("cgroup.procs")).map_err(|cause| format!("`{}` could not be read: {cause}", own.display()))?;

        let mut remaining = 0_usize;

        for pid in listed.split_ascii_whitespace() {
            // #[gamma::skip(literal.int_increment, reason = "remaining is used only as zero versus nonzero, so adding any positive value for each listed pid is equivalent")]
...
impl Cgroup {
    ...
    fn create_under_with(root: &Path, limit: Option<u64>, pid: u32, next: impl FnMut() -> u64) -> Result<Self, String> {
        ...
        let group = Self { path, tracked: false };
...
mod tests {
    ...
    /// An emptied cgroup is reported as vacated.
    #[test]
    fn a_cgroup_whose_processes_have_left_is_vacated() {
    ...
    /// A cgroup that will not empty is refused rather than waited on forever.
    ///
    /// The retry loop exists because a process can be reaped or spawned between the listing and
    /// the write, but it is bounded: a cgroup holding something that will not move has to become
    /// an error, since cgroup v2 refuses to hand the memory controller to the children of a
    /// non-empty cgroup and the run would otherwise install no ceiling at all.
    #[test]
    fn a_cgroup_that_will_not_empty_is_refused() {
        let directory = tempfile::tempdir().expect("a temporary directory");
        let own = directory.path().join("own");
        let supervisor = directory.path().join("supervisor");
        ...
        fs::create_dir_all(&own).expect("created");
        fs::create_dir_all(&supervisor).expect("created");
        fs::write(own.join("cgroup.procs"), "101\n102\n").expect("written");
        fs::write(supervisor.join("cgroup.procs"), "").expect("written");

        let refusal = vacate(&own, &supervisor).expect_err("a cgroup that never empties cannot be used");

        assert!(refusal.contains("still holds processes"), "{refusal}");
    }
