...
//! unmutated baseline, then tests every mutant in parallel and prints the ones that survived:
//!
//! ```text
//!     SURVIVED src/parse.rs:88:21: replace < with <= [relational.lt_to_le]
//!     SURVIVED src/cache.rs:41:9: delete self.hits += 1 [stmt.delete_assign]
//!      TIMEOUT src/scan.rs:12:16: replace + with - [arith.add_to_sub]
//!        Found 303 mutants in 41 files
//!      Summary 303 mutants (294 killed, 7 survived, 2 timed out, 0 out of memory, 0 uncovered => 97.7%)
//! ```
//!
//! Every line names the mutator that produced it in brackets. That name is the whole vocabulary:
//! it is what you pass to `--mutators`, what you write in a suppression, what `explain` describes, and
...
//! cargo gamma explain <subject>  # describe a mutator, a family, a preset, or a mutant id
//! cargo gamma suppress     # write suppressions for mutants that cannot usefully be tested
//! cargo gamma merge <reports>  # combine per-shard reports into one score
//! cargo gamma migrate      # translate a foreign project's settings into this one's vocabulary
//! cargo gamma completions bash  # print a shell completion script
//! ```
//!
//! ## run
//!
...
//! cargo gamma list mutators         # the mutator catalog, marking the ones currently enabled
...
//!
//! ```text
//! cargo gamma list --mutators '@arithmetic' --file 'src/money/**'
//! ```
//!
//! ## explain
//!
//! Turns a name from a report back into prose — a mutator, a family, a preset, an academic alias,
...
//!
//! ```text
//! cargo gamma suppress --dry-run-suppress  # show the edits without making them
...
//! ## migrate
//!
//! Translates a `.cargo/mutants.toml` configuration file, or the command line that goes with it,
//! into this tool's vocabulary, so that a project arriving from another mutation tester does not
//! have to be re-configured by hand. `#[mutants::skip]` attributes are not translated and not
//! honored; rewrite them as `#[gamma::skip]`.
//!
//! ```text
...
//!
//! `cargo gamma list mutators` prints all of them with a marker beside the ones the current selection
//! enables, and `cargo gamma explain <name>` describes any one of them.
//!
//! ## Selecting mutators
//!
//! `--mutators` takes a comma-separated selector list. A selector is a full mutator name, a family
//! prefix, an `@preset`, an academic alias, or `all`; `!` in front removes rather than adds, and
//! selectors apply left to right, so a list reads as a sentence.
//!
//! ```text
//! cargo gamma run --mutators relational.lt_to_le      # one mutator
//! cargo gamma run --mutators arith,bitwise            # two families
//! cargo gamma run --mutators '@arithmetic'            # a preset
//! cargo gamma run --mutators ROR                      # an academic alias
//! cargo gamma run --mutators 'all,!stmt'              # everything except statement deletion
//! cargo gamma run --mutators '@default,literal'       # the default set, plus one more family
//! ```
//!
...
//! | `@pedantic` | additional low-yield mutations excluded from the default selection |
...
//! | `@extreme` | every registered mutator, including `@pedantic` |
//!
...
//! cargo gamma run --workspace                      # every package
...
//! #[gamma::expect_survived(literal, reason = "log text is not asserted on")]
...
//! cargo gamma run --html-report target/mutants.html    # self-contained interactive page
//! cargo gamma run --sarif-report target/mutants.sarif  # SARIF 2.1.0, for code scanning
...
//! ```
//!
//! Incremental execution is enabled by default (`--incremental full`), carrying forward settled test
//! verdicts and unviability from previous runs when files are unchanged:
//!
//! ```text
//! cargo gamma run --incremental full
//! ```
//!
//! Pass `--incremental no` to force a cold run from scratch.
//!
//! # Configuration
//!
...
//!
//! ```toml
//! mutators      = ["@arithmetic", "@boundary", "stmt"]
...
//! cargo gamma run --test-timeout-multiplier 3  # multiple of each test binary's baseline duration
...
//! the console rendering, which a fake [`cargo_gamma_lib::Host`] turns into ordinary assertions.
//!
