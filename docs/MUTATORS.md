...
Replaces an entire function body with a fixed value of the same type: a default, a small literal, `None`, an empty collection, and so on. A surviving mutant here says that nothing calling this function checks its return value at all — the function could be replaced by a constant and the suite would not notice.
