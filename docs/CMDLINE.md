...
| [`gamma migrate`](#gamma-migrate) | Translate a foreign mutation-testing project to cargo-gamma |
...
Three categories are specific to one subcommand: **Suppressing** (`suppress`, `unsuppress`),
**Merging** (`merge`) and **Migrating** (`migrate`).
...
2. **The configuration file**, `.cargo/gamma.toml` by default — see [CONFIG.md](CONFIG.md) for every
...
| `-p`, `--package` | `<NAME>` | Only mutate these packages. Defaults to the package the current directory belongs to. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `--config` | `<PATH>` | Read configuration from this file instead of `.cargo/gamma.toml`. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `--scratch-dir` | `<PATH>` | Put the scratch tree and its build artifacts here instead of under the workspace's `target`. |
| `--copy-ignored` |  | Copy files version control ignores into the scratch tree as well. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `--min-score` | `<PERCENT>` | Fail the run if the mutation score is below this percentage. |
| `--incremental` | `<INCREMENTAL>` | How much an incremental run reuses from the last run: `no` (cold run), `build` (compiler unviability cache only), `full` (compiler and test verdicts, validated by workspace snapshots). |
| `--leak-dirs` |  | Keep the scratch tree after the run and say where it is. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `--killed` |  | List the mutants the suite killed, not just the ones that survived. |
| `--unviable` |  | List every mutant that could not be compiled, not just how many there were. |
| `--html-report` | `<PATH>` | Write the self-contained HTML report somewhere other than the default. |
| `--json-report` | `<PATH>` | Write the `mutation-testing-elements` JSON report somewhere other than the default. |
| `--html-external` |  | Load the report viewer from a CDN instead of embedding it. |
| `--sarif-report` | `<PATH>` | Write a SARIF 2.1.0 log of the surviving mutants to this path. |
...
| `--advice` | `<PATH>` | Write the Markdown diagnosis somewhere other than the default. |
| `--diag-bundle` | `<PATH>` | Write the diagnostics bundle somewhere other than the default. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `-d`, `--dir` | `<PATH>` | Path to the project to migrate. Defaults to `.`. |
| `--command` | `<ARG>` | Translate a foreign command line instead of a configuration file, printing POSIX-shell-safe words. |

**Run control**

| Option | Value | What it does |
| --- | --- | --- |
| `--dry-run` |  | Print the translation without writing anything. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `--min-score` | `<PERCENT>` | Fail if the merged score is below this percentage. |
...
| Option | Value | What it does |
| --- | --- | --- |
| `--scratch-dir` | `<PATH>` | Read the run record from here instead of the workspace's `target`. |
